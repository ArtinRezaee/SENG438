 ...
 */
public class SpiderWebPlot extends Plot implements Block, Cloneable, Serializable {
	...
	/**
	 * The default maximum value plotted - forces the plot to evaluate the maximum
	...
	/**
	 * The data extract order (BY_ROW or BY_COLUMN). This denotes whether the data
	 * series are stored in rows (in which case the category names are derived from
	 * the column keys) or in columns (in which case the category names are derived
	 * from the row keys).
	 */
	 ...
	 *            the dataset.
	 * @param extract
	 *            controls how data is extracted ({@link TableOrder#BY_ROW} or
	 *            {@link TableOrder#BY_COLUMN}).
	 */
	...
	/**
	 * Sets the webFilled flag and sends a {@link PlotChangeEvent} to all registered
	 * listeners.
	 * 
	 * @param flag
	 *            the flag.
	...
	/**
	 * Sets the head percent and sends a {@link PlotChangeEvent} to all registered
	 * listeners.
	 * 
	 * @param percent
	 *            the percent.
	...
	/**
	 * Returns the start angle for the first radar axis. <BR>
	 * This is measured in degrees starting from 3 o'clock (Java Arc2D default) and
	 ...
	 * listeners. This controls the space between the edges of the plot and the plot
	 * area itself (the region where the axis labels appear).
	 * 
	 * @param percent
	...
	/**
	 * Sets the axis label gap and sends a {@link PlotChangeEvent} to all registered
	 * listeners.
	 * 
	 * @param gap
	 *            the gap.
	 ...
	 * </code>, then a list of paints is used instead (to allow different colors to
	 * be used for each series of the radar group).
	 * 
	 * @param paint
	...
	/**
	 * Sets the outline paint for ALL series in the plot. If this is set to </code>
	 * null</code>, then a list of paints is used instead (to allow different colors
	 * to be used for each series).
	 * 
	 * @param paint
	 *            the paint (<code>null</code> permitted).
	 */
	...
	/**
	 * Sets the outline stroke for ALL series in the plot. If this is set to </code>
	 * null</code>, then a list of paints is used instead (to allow different colors
	 * to be used for each series).
	 * 
	 * @param stroke
	...
	/**
	 * Sets the tool tip generator for the plot and sends a {@link PlotChangeEvent}
	...
	/**
	 * Sets the URL generator for the plot and sends a {@link PlotChangeEvent} to
			...
			if (dataValue != null) {
				...
				if (value >= 0) { // draw the polygon series...
					...
					if (entities != null) {
						...
						CategoryItemEntity entity = new CategoryItemEntity(area, tip, url, this.dataset, series,
	 ...
	 * category. This allows us to plot BY_ROW or BY_COLUMN which basically is just
	 * reversing the definition of the categories and data series being plotted
	 * 
	 * @param series
	 ...
	 */
	protected Point2D calculateLabelLocation(Rectangle2D labelBounds, double ascent, Rectangle2D plotArea,
			double startAngle) {
