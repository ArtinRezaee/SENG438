...
public class AxisPlotArrangement implements Arrangement, Serializable {
	...
	/**
	 * Coordinates defining the remaining space for the plot after the axes have
	 * been allocated space - this is where the data is plotted.
	 */
	...
	/**
	 * Returns the x-coordinate that marks the line between the left axes and the
	 * data area.
	 * 
	...
	/**
	 * Returns the x-coordinate that marks the line between the right axes and the
	 * data area.
	 * 
	 * @return The x-coordinate.
	 */
	...
	/**
	 * Returns the y-coordinate that marks the line between the top axes and the
	 * data area.
	 * 
	...
	/**
	 * Returns the y-coordinate that marks the line between the bottom axes and the
	 * data area.
	 * 
	 * @return The y-coordinate.
	 */
	...
	/**
	 * Arranges the components in the specified container and returns the resulting
	 * size.
	 * 
	 * @param container
	 *            the container.
	...
	protected ArrangeResult arrangeNN(BlockContainer container, Graphics2D g2, ArrangeParams params) {
		...
		ArrangeResult ar1 = this.centerBlock.arrange(g2, RectangleConstraint.NONE, params);
	...
	protected ArrangeResult arrangeFF(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
		...
		this.x1 = constraint.getWidth() - margin.getLeft() - margin.getRight() - border.getInsets().getLeft()
				- border.getInsets().getRight() - padding.getLeft() - padding.getRight();
		this.y0 = 0.0; // margin.getTop() + border.getInsets().getTop() + padding.getTop();
		this.y1 = constraint.getHeight() - margin.getTop() - margin.getBottom() - border.getInsets().getTop()
				- border.getInsets().getBottom() - padding.getTop() - padding.getBottom();

		RectangleConstraint c1 = new RectangleConstraint(new Range(0.0, this.x1), this.y1);
		...
		for (int i = 0; i < this.leftBlocks.size(); i++) {
			...
			b.setBounds(new Rectangle2D.Double(this.x0, this.y0, size.getWidth(), size.getHeight()));
		...
		for (int i = 0; i < this.rightBlocks.size(); i++) {
			...
			ArrangeResult r = b.arrange(g2, c1, null);
			Size2D size = r.getSize();
			b.setBounds(new Rectangle2D.Double(this.x1 - size.getWidth(), this.y0, size.getWidth(), size.getHeight()));
			this.x1 = this.x1 - size.getWidth();
		}

		RectangleConstraint c2 = new RectangleConstraint(this.x1 - this.x0, new Range(0.0, this.y1 - this.y0));
		...
		for (int i = 0; i < this.bottomBlocks.size(); i++) {
			...
			ArrangeResult r = b.arrange(g2, c2, null);
			Size2D size = r.getSize();
			b.setBounds(new Rectangle2D.Double(this.x0, this.y1 - size.getHeight(), size.getWidth(), size.getHeight()));
		...
		for (int i = 0; i < this.leftBlocks.size(); i++) {
			...
			bounds.setRect(bounds.getX(), this.y0, bounds.getWidth(), this.y1 - this.y0);
			b.setBounds(bounds);
		}
	...
	/**
	 * Tests this <code>AxisPlotArrangement</code> for equality with an arbitrary
	 * object.
	 * 
	 * @param obj
