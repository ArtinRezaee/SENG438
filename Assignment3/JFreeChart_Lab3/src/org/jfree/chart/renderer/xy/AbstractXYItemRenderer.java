 ...
 *                   Tim Bardzil;
 *
 * $Id: AbstractXYItemRenderer.java,v 1.29 2005/12/12 14:11:32 mungady Exp $
 *
 * Changes:
 * --------
...
public abstract class AbstractXYItemRenderer extends AbstractRenderer
	 ...
	 * passes control to the {@link #getSeriesItemLabelGenerator(int)} method. If,
	 * for some reason, you want a different generator for individual items, you can
	...
	/**
	 * Sets the radius of the circle used for the default entity area when no area
	 * is specified.
	 * 
	 * @param radius
	 *            the radius.
	 ...
	 * 
	 * @return The range (<code>null</code> if the dataset is <code>null</code> or
	 *         empty).
	 */
	...
	public LegendItem getLegendItem(int datasetIndex, int series) {
		...
		if (xyplot != null) {
			...
			if (dataset != null) {
				XYSeriesLabelGenerator llg = getLegendItemLabelGenerator();
				String label = llg.generateLabel(dataset, series);
				AttributedString attributedLabel = llg.generateAttributedLabel(dataset, series);
				String description = label;
				String toolTipText = null;

				XYSeriesLabelGenerator ttg = getLegendItemToolTipGenerator();
				if (ttg != null) {
					toolTipText = ttg.generateLabel(dataset, series);
				}
				String urlText = null;
				...
				if (attributedLabel != null) {
					result = new LegendItem(attributedLabel, description, toolTipText, urlText, shape, paint,
	 ...
	 */
	protected Point2D calculateDomainMarkerTextAnchorPoint(Graphics2D g2, PlotOrientation orientation,
	 ...
	 * @return The coordinates for drawing the marker label.
	 */
	private Point2D calculateRangeMarkerTextAnchorPoint(Graphics2D g2, PlotOrientation orientation,
			Rectangle2D dataArea, Rectangle2D markerArea, RectangleInsets markerOffset,
	 ...
	 *            the item.
	 * @param entityX
	 *            the entity's center x-coordinate in user space.
	 * @param entityY
	...
	protected void addEntity(EntityCollection entities, Shape area, XYDataset dataset, int series, int item,
		...
		if (area == null) {
			area = new Ellipse2D.Double(entityX - this.defaultEntityRadius, entityY - this.defaultEntityRadius,
