 ...
 *                   Nicolas Brodu;
 *
 * $Id: JFreeChart.java,v 1.40 2005/11/23 14:17:31 mungady Exp $
 *
 ...
 * {@link Plot} and a {@link org.jfree.data.general.Dataset} (the plot in turn
 ...
 *
 */
public class JFreeChart extends AbstractContentBlock
		implements Block, TitleChangeListener, PlotChangeListener, Serializable, Cloneable {
	 ...
	 * <br>
	 * <br>
	 * Note that the {@link ChartFactory} class contains a range of static methods
	 * that will return ready-made charts, and often this is a more convenient way
	 * to create charts than using this constructor.
	 *
	 * @param plot
	...
	public JFreeChart(Plot plot) {
		...
		this(null, // title
				null, // font
				plot, true // create legend
		);

	}

	/**
	 * Creates a new chart with the given title and plot. A default font (@link
	 * DEFAULT_TITLE_FONT) is used for the title, and the chart will have a legend
	 ...
	 * <code>createLegend</code> argument specifies whether or not a legend should
	 * be added to the chart. <br>
	 * <br>
	 * Note that the {@link ChartFactory} class contains a range of static methods
	 * that will return ready-made charts, and often this is a more convenient way
	 * to create charts than using this constructor.
	...
	public JFreeChart(String title, Font titleFont, Plot plot, boolean createLegend) {
		...
		this.container = new BlockContainer(new StandardArrangement(this));
		...
		if (createLegend) {
			...
			legend.setInteriorBackgroundPaint(Color.white);
		...
		// by default the container should handle all chart layout, but
		// we'll allow callers to grab the container and set it up in whatever
		// way they choose...
	 ...
	 */
	public BlockContainer getLayoutContainer() {
	...
	/**
	 * Returns the main chart title. Very often a chart will have just one title, so
	 * we make this case simple by providing accessor methods for the main title.
	 * However, multiple titles are supported - see the {@link #addSubtitle(Title)}
	 * method.
	 *
	...
	/**
	 * Sets the main title for the chart and sends a {@link ChartChangeEvent} to all
	 * registered listeners. If you do not want a title for the chart, set it to
	 * <code>null</code>. If you want more than one title on a chart, use the
	...
	/**
	 * Sets the chart title and sends a {@link ChartChangeEvent} to all registered
	 * listeners. This is a convenience method that ends up calling the
	 * {@link #setTitle(TextTitle)} method. If there is an existing title, its text
	 * is updated, otherwise a new title using the default font is added to the
	 * chart. If <code>text</code> is <code>null</code> the chart title is set to
	 * <code>null</code>.
	 *
	 * @param text
	...
	/**
	 * Returns the legend for the chart, if there is one. Note that a chart can have
	 * more than one legend - this method returns the first.
	 * 
	...
	/**
	 * Removes the first legend in the chart and sends a {@link ChartChangeEvent} to
	...
	/**
	 * Clears all subtitles from the chart and sends a {@link ChartChangeEvent} to
	 * all registered listeners.
	 */
	...
	/**
	 * Removes the specified subtitle and sends a {@link ChartChangeEvent} to all
	 * registered listeners.
	 * 
	 * @param title
	 *            the title.
	...
	/**
	 * Sets the paint used to fill the chart interior background and sends a
	 * {@link ChartChangeEvent} to all registered listeners.
	 *
	 * @param paint
	 *            the paint (<code>null</code> permitted).
	...
	public ArrangeResult arrange(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params) {
		...
		for (int i = 0; i < this.subtitles.size(); i++) {
			this.container.add(getSubtitle(i));
		}
		if (this.plot != null) {
			this.container.add(this.plot);
		}
		return this.container.arrange(g2, constraint, params);
	}

	/**
	 * Draws the chart within the specified area.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param area
	 ...
	 *            optional parameters.
	 * 
	 * @return A result (currently <code>null</code>).
	 */
	...
	public void draw(Graphics2D g2, Rectangle2D chartArea, Point2D anchor, ChartRenderingInfo info) {
		...
		// clone the chart area to make sure we don't change the original
		...
		g2.addRenderingHints(getRenderingHints());
		...
		if (getBackgroundImage() != null) {
			...
			g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, getBackgroundImageAlpha()));
			Rectangle2D dest = new Rectangle2D.Double(0.0, 0.0, getBackgroundImage().getWidth(null),
					getBackgroundImage().getHeight(null));
			Align.align(dest, chartArea, getBackgroundImageAlignment());
			g2.drawImage(getBackgroundImage(), (int) dest.getX(), (int) dest.getY(), (int) dest.getWidth(),
					(int) dest.getHeight(), null);
			g2.setComposite(originalComposite);
		}

		// draw the title and subtitles...
		this.container.draw(g2, chartArea);
	 ...
	 * @return A rectangle.
	 */
	protected Rectangle2D createAlignedRectangle2D(Size2D dimensions, Rectangle2D frame, HorizontalAlignment hAlign,
	...
	protected EntityCollection drawTitle(Title t, Graphics2D g2, Rectangle2D area, boolean entities) {
		...
		if (position == RectangleEdge.TOP) {
			ArrangeResult r = t.arrange(g2, constraint, null);
...
class JFreeChartInfo extends ProjectInfo {
	...
	public JFreeChartInfo() {
		...
		setContributors(Arrays.asList(new Contributor[] { new Contributor("Eric Alexander", "-"),
				...
				new Contributor("Brian Cole", "-"), new Contributor("Pascal Collet", "-"),
				new Contributor("Martin Cordova", "-"), new Contributor("Paolo Cova", "-"),
				...
				new Contributor("Tobias Selb", "-"), new Contributor("Mofeed Shahin", "-"),
				new Contributor("Pady Srinivasan", "-"), new Contributor("Greg Steckman", "-"),
				...
				new Contributor("Sylvain Vieujot", "-"), new Contributor("Mark Watson", "www.markwatson.com"),
