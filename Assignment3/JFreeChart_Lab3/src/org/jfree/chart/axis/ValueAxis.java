 ...
 *                   Center);
 *
 * $Id: ValueAxis.java,v 1.14 2005/07/19 14:27:53 mungady Exp $
 *
 ...
 */
public abstract class ValueAxis extends Axis implements Block, Cloneable, Serializable {
	...
	private RectangleEdge fixedAxisEdge;

	/**
	 * The current list of ticks for the axis (updated in the arrange() method).
	 */
	...
	/**
	 * Returns <code>true</code> if the tick labels should be rotated (to vertical),
	 ...
	 * (that is, rotated 90 degrees from horizontal). If the flag is changed, an
	...
	/**
	 * Returns the list of ticks for the axis.
	 * 
	 * @return The list of ticks (unmodifiable).
	 */
	...
	/**
	 * Returns the axis location relative to the plot. Usually the plot keeps track
	 * of the axis location, but this setting overrides the plot.
	 * 
	 * @return The axis location (possibly <code>null</code>).
	 */
	public RectangleEdge getFixedAxisEdge() {
		return this.fixedAxisEdge;
	}

	/**
	 * Sets the axis location relative to the plot. Usually the plot keeps track of
	 * the axis location, but you can override the plot with this value.
	 * 
	 * @param location
	 *            the location.
	 */
	public void setFixedAxisEdge(RectangleEdge location) {
		this.fixedAxisEdge = location;
	}
	 ...
	 *            the axis location.
	 */
	protected abstract void selectAutoTickUnit(Graphics2D g2, Rectangle2D dataArea, RectangleEdge edge);

	/**
	 * Returns the location of this axis relative to the plot.
	 * 
	 * @return The location.
	 */
	protected RectangleEdge getAxisEdge() {
		RectangleEdge result = this.fixedAxisEdge;
		if (result == null) {
			Plot p = getPlot();
			if (p instanceof XYPlot) {
				XYPlot xyp = (XYPlot) p;
				result = xyp.getAxisEdge(this);
			} else if (p instanceof CategoryPlot) {
				CategoryPlot cp = (CategoryPlot) p;
				result = cp.getAxisEdge(this);
			}
		}
		return result;
	}
	...
	/**
	 * Arranges the axis subject to the given constraint.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param constraint
	...
	public ArrangeResult arrange(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params) {
		...
		ArrangeResult r = null;
		LengthConstraintType w = cc.getWidthConstraintType();
		LengthConstraintType h = cc.getHeightConstraintType();
		if (w == LengthConstraintType.NONE) {
			if (h == LengthConstraintType.NONE) {
				r = arrangeNN(g2, params);
			} else if (h == LengthConstraintType.FIXED) {
				r = arrangeNF(g2, cc.getHeight(), params);
			} else if (h == LengthConstraintType.RANGE) {
				throw new RuntimeException("Not implemented.");
			}
		} else if (w == LengthConstraintType.FIXED) {
			if (h == LengthConstraintType.NONE) {
				r = arrangeFN(g2, cc.getWidth(), params);
			} else if (h == LengthConstraintType.FIXED) {
				r = arrangeFF(g2, cc.getWidth(), cc.getHeight(), params);
			} else if (h == LengthConstraintType.RANGE) {
				r = arrangeFR(g2, cc.getWidth(), cc.getHeightRange(), params);
			}
		} else if (w == LengthConstraintType.RANGE) {
			if (h == LengthConstraintType.NONE) {
				throw new RuntimeException("Not implemented.");
			} else if (h == LengthConstraintType.FIXED) {
				r = arrangeRF(g2, cc.getWidthRange(), cc.getHeight(), params);
			} else if (h == LengthConstraintType.RANGE) {
				throw new RuntimeException("Not implemented.");
			}
		}
		r.setSize(calculateTotalWidth(r.getWidth()), calculateTotalHeight(r.getHeight()));
		return r;
	}

	/**
	 * Returns the size of the axis given no constraints. In practice, this can only
	 * be done if the length of the axis is specified (we assume 10.0 if the length
	 * hasn't been set by giving the axis a fixed height or width depending on where
	 * it is located).
	 * 
	 * @param g2
	 *            the graphics device.
	 ...
	 * 
	 * @return The layout result (includes the *content* size).
	 */
	protected ArrangeResult arrangeNN(Graphics2D g2, ArrangeParams params) {
		RectangleEdge edge = getAxisEdge();
		if (RectangleEdge.isLeftOrRight(edge)) {
			double h = getDefaultHeight();
			if (h < 0.0) {
				h = 10.0; // a default *total* height
			}
			return arrangeNF(g2, trimToContentHeight(h), params);
		} else { // top or bottom
			double w = getDefaultWidth();
			if (w < 0.0) {
				w = 10.0; // a default *total* width
			}
			return arrangeFN(g2, trimToContentWidth(w), params);
		}
	}

	/**
	 * Returns the size of the axis given a fixed height constraint.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param fixedHeight
	...
	protected ArrangeResult arrangeNF(Graphics2D g2, double fixedHeight, ArrangeParams params) {
		// TODO: use different code for top/bottom vs left/right
		RectangleEdge edge = getAxisEdge();
		Rectangle2D dataArea = new Rectangle2D.Double(0.0, 0.0, 1.0, fixedHeight);
		if (isAutoTickUnitSelection()) {
			selectAutoTickUnit(g2, dataArea, edge);
		}
		this.ticks = refreshTicks(g2, null, dataArea, edge);
		double w = getDefaultWidth(); // has a width been set?
		if (w < 0.0) {
			Rectangle2D labelBounds = getLabelEnclosure(g2, edge);
			w = this.findMaximumTickLabelWidth(this.ticks, g2, dataArea, isVerticalTickLabels());
			w = w + labelBounds.getWidth();
		} else {
	...
	/**
	 * Returns the size of the axis given a fixed width constraint.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param fixedWidth
	 ...
	 *            the layout parameters (<code>null</code> not permitted).
	 * 
	 * @return The layout result (including the *content* size).
	 */
	protected ArrangeResult arrangeFN(Graphics2D g2, double fixedWidth, ArrangeParams params) {
		RectangleEdge edge = getAxisEdge();
		// TODO: separate code to handle TOP/BOTTOM vs LEFT/RIGHT
		Rectangle2D dataArea = new Rectangle2D.Double(0.0, 0.0, fixedWidth, 1.0);
		if (isAutoTickUnitSelection()) {
			selectAutoTickUnit(g2, dataArea, edge);
		}
		this.ticks = refreshTicks(g2, null, dataArea, edge);
		double h = getDefaultHeight(); // has a height been specified?
		if (h < 0.0) {
			Rectangle2D labelBounds = getLabelEnclosure(g2, edge);
			h = findMaximumTickLabelHeight(this.ticks, g2, dataArea, isVerticalTickLabels());
			h = h + labelBounds.getHeight();
		} else {
	...
	/**
	 * Arranges the axis with a fixed width and a fixed height. We need to check
	 * with the plot that owns the axis to find out which edge of the plot the axis
	 * is located on.
	 * 
	 * @param g2
	 *            the graphics device.
	...
	protected ArrangeResult arrangeFF(Graphics2D g2, double fixedWidth, double fixedHeight, ArrangeParams params) {
		...
		RectangleEdge edge = getAxisEdge();
		// we create a 'dummy' data area of the appropriate width just for
		// the auto tick calculation - later we can probably eliminate the
		// need for this
		Rectangle2D dataArea = new Rectangle2D.Double(0.0, 0.0, fixedWidth, fixedHeight);
		if (isAutoTickUnitSelection()) {
			selectAutoTickUnit(g2, dataArea, edge);
		}
		this.ticks = refreshTicks(g2, null, dataArea, edge);
	...
	/**
	 * Arranges the axis with a fixed width and a variable height. This usually
	 * means the axis is horizontal, but it doesn't have to be that way.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param fixedWidth
	 ...
	 * @return The *content* size.
	 */
	protected ArrangeResult arrangeFR(Graphics2D g2, double fixedWidth, Range heightRange, ArrangeParams params) {

		RectangleEdge edge = getAxisEdge();

		// we create a 'dummy' data area of the appropriate width just for
		// the auto tick calculation - with some new axis translation methods
		// we can probably eliminate the need for this
		Rectangle2D dataArea = new Rectangle2D.Double(0.0, 0.0, fixedWidth, 1.0);
		if (isAutoTickUnitSelection()) {
			selectAutoTickUnit(g2, dataArea, edge);
		}
		this.ticks = refreshTicks(g2, null, dataArea, edge);
		double h = heightRange.getUpperBound();
		if (RectangleEdge.isTopOrBottom(edge)) {
			Rectangle2D labelBounds = getLabelEnclosure(g2, edge);
			h = this.findMaximumTickLabelHeight(this.ticks, g2, dataArea, isVerticalTickLabels());
			h = h + labelBounds.getHeight();
		}
	...
	/**
	 * Arranges the axis with a variable width and a fixed height. This usually
	 * means the axis is vertical, but it doesn't have to be that way.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param widthRange
	 ...
	 * @return The *content* size.
	 */
	protected ArrangeResult arrangeRF(Graphics2D g2, Range widthRange, double fixedHeight, ArrangeParams params) {
		...
		this.ticks = refreshTicks(g2, null, dataArea, edge);
		double w = widthRange.getUpperBound();
		if (RectangleEdge.isLeftOrRight(edge)) {
			Rectangle2D labelBounds = getLabelEnclosure(g2, edge);
			w = this.findMaximumTickLabelWidth(this.ticks, g2, dataArea, isVerticalTickLabels());
			w = w + labelBounds.getWidth();
	...
	/**
	 * Draws the axis. This method is defined in the
	 * {@link org.jfree.chart.block.Block} interface, that will eventually be
	 * implemented by all axes.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param area
	 *            the axis area.
	 * @param params
	 *            incoming parameters which are ignored at this point (possibly
	 *            <code>null</code>).
	 * 
	 * @return Results from drawing the axis (<code>null</code> at present).
	 */
	...
	/**
	 * Draws the axis within the specified area.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param area
	...
	public void draw(Graphics2D g2, Rectangle2D area) {
		...
		Paint ibg = getInteriorBackgroundPaint();
		if (ibg != null) {
			g2.setPaint(ibg);
			g2.fill(area);
		}
		...
		RectangleEdge edge = getAxisEdge();
		draw(g2, area, edge, null);
	}

	/**
	 * Draws an axis line against one edge of the axis area.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param axisArea
	 ...
	 * @param location
	 *            the location of the axis relative to the plot.
	 */
	protected void drawAxisLine(Graphics2D g2, Rectangle2D axisArea, RectangleEdge location) {
		Line2D axisLine = null;
		RectangleEdge edge = RectangleEdge.opposite(location);
		if (edge == RectangleEdge.TOP) {
			axisLine = new Line2D.Double(axisArea.getX(), axisArea.getY(), axisArea.getMaxX(), axisArea.getY());
		} else if (edge == RectangleEdge.BOTTOM) {
			axisLine = new Line2D.Double(axisArea.getX(), axisArea.getMaxY(), axisArea.getMaxX(), axisArea.getMaxY());
		} else if (edge == RectangleEdge.LEFT) {
			axisLine = new Line2D.Double(axisArea.getX(), axisArea.getY(), axisArea.getX(), axisArea.getMaxY());
		} else if (edge == RectangleEdge.RIGHT) {
			axisLine = new Line2D.Double(axisArea.getMaxX(), axisArea.getY(), axisArea.getMaxX(), axisArea.getMaxY());
		}
	 ...
	 *            the tick.
	 * @param area
	 *            the area within which the axis is being drawn.
	 * @param edge
	 *            the edge along which the ticks are drawn.
	 * 
	 * @return The x and y coordinates of the anchor point.
	 */
	protected float[] calculateAnchorPoint(ValueTick tick, Rectangle2D area, RectangleEdge edge) {

		RectangleInsets insets = getTickLabelInsets();
		float[] result = new float[2];
		if (edge == RectangleEdge.TOP) {
			result[0] = (float) valueToJava2D(tick.getValue(), area, edge);
			result[1] = (float) (area.getMaxY() - insets.getBottom() - 2.0);
		} else if (edge == RectangleEdge.BOTTOM) {
			result[0] = (float) valueToJava2D(tick.getValue(), area, edge);
			result[1] = (float) (area.getY() + insets.getTop() + 2.0);
		} else if (edge == RectangleEdge.LEFT) {
			result[0] = (float) (area.getMaxX() - insets.getRight() - 2.0);
			result[1] = (float) valueToJava2D(tick.getValue(), area, edge);
		} else if (edge == RectangleEdge.RIGHT) {
			result[0] = (float) (area.getX() + insets.getLeft() + 2.0);
			result[1] = (float) valueToJava2D(tick.getValue(), area, edge);
		}
		return result;
	}
