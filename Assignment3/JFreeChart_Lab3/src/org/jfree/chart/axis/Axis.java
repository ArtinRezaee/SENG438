 ...
 * ({@link CategoryAxis}).
 */
public abstract class Axis extends AbstractContentBlock implements Cloneable, Serializable {
	...
	/**
	 * Sets the stroke used to draw tick marks and sends an {@link AxisChangeEvent}
	 * to all registered listeners.
	 *
	 * @param stroke
	 *            the stroke (<code>null</code> not permitted).
	...
	/**
	 * Sets the paint used to draw tick marks and sends an {@link AxisChangeEvent}
	 * to all registered listeners.
	 *
	 * @param paint
	 *            the paint (<code>null</code> not permitted).
	 ...
	 */
	public abstract AxisState draw(Graphics2D g2, Rectangle2D area, RectangleEdge location,
	 ...
	 *            the edge.
	 */
	private void alignRectToEdge(Rectangle2D rect, Rectangle2D frame, RectangleEdge edge) {
		double x = 0.0;
		double y = 0.0;
	 ...
	 *            the graphics device.
	 * @param axisArea
	 *            the area in which to draw the axis.
	 * @param edge
	 *            the edge of the axis area that the axis provides the scale for.
	 */
	protected void drawLabel(String label, Graphics2D g2, Rectangle2D axisArea, RectangleEdge edge) {
	 ...
	 *            the edge.
	 */
	protected void drawAxisLine(Graphics2D g2, Rectangle2D area, RectangleEdge edge) {

		Line2D axisLine = null;
		if (edge == RectangleEdge.TOP) {
			axisLine = new Line2D.Double(area.getX(), area.getY(), area.getMaxX(), area.getY());
		} else if (edge == RectangleEdge.BOTTOM) {
			axisLine = new Line2D.Double(area.getX(), area.getMaxY(), area.getMaxX(), area.getMaxY());
		} else if (edge == RectangleEdge.LEFT) {
			axisLine = new Line2D.Double(area.getX(), area.getY(), area.getX(), area.getMaxY());
		} else if (edge == RectangleEdge.RIGHT) {
			axisLine = new Line2D.Double(area.getMaxX(), area.getY(), area.getMaxX(), area.getMaxY());
		}
