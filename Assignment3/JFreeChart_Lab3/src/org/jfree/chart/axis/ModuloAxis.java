 ...
 * Contributor(s):   -;
 *
 * $Id: ModuloAxis.java,v 1.3 2005/03/16 09:41:11 mungady Exp $
 *
 * Changes
 * -------
...
public class ModuloAxis extends NumberAxis {
	...
	/**
	 * The fixed range for the axis - all data values will be mapped to this range
	 * using a modulo calculation.
	 */
	private Range fixedRange;

	/**
	 * The display start value (this will sometimes be > displayEnd, in which case
	 * the axis wraps around at some point in the middle of the axis).
	 */
	 ...
	 * @param fixedRange
	 *            the fixed range (<code>null</code> not permitted).
	 */
	...
	/**
	 * This method should calculate a range that will show all the data values. For
	 * now, it just sets the axis range to the fixedRange.
	 */
	...
	private double transStart(double value, Rectangle2D area, RectangleEdge edge, double length1, double length2) {
		...
		if (isInverted()) {
			return max - ((value - this.displayStart) / (this.fixedRange.getUpperBound() - this.displayStart))
					* (max - min);
		} else {
			return min + ((value - this.displayStart) / (this.fixedRange.getUpperBound() - this.displayStart))
					* (max - min);
		}
	...
	private double transEnd(double value, Rectangle2D area, RectangleEdge edge, double length1, double length2) {
		...
		if (isInverted()) {
			return max
					- ((value - this.fixedRange.getLowerBound()) / (this.displayEnd - this.fixedRange.getLowerBound()))
							* (max - min);
		} else {
			return min
					+ ((value - this.fixedRange.getLowerBound()) / (this.displayEnd - this.fixedRange.getLowerBound()))
