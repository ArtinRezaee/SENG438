 ...
 *
 * $Id: CyclicNumberAxis.java,v 1.11 2005/06/17 09:54:55 mungady Exp $
 *
 * Changes
 * -------
...
public class CyclicNumberAxis extends NumberAxis {
	 ...
	 *            the data area.
	 * @param edge
	 *            the side of the rectangle on which the axis is displayed.
	 */
	...
	protected static class CycleBoundTick extends NumberTick {
		 ...
		 *            the rotation angle.
		 */
		public CycleBoundTick(boolean mapToLastCycle, Number number, String label, TextAnchor textAnchor,
	 ...
	 *            the data area.
	 * @param edge
	 *            the side on which the axis is displayed.
	 * 
			...
			if (isVerticalTickLabels()) {
				...
				if ((lastTick != null) && (lastX == x) && (currentTickValue != cycleBound)) {
					...
					result.add(new CycleBoundTick(this.boundMappedToLastCycle, lastTick.getNumber(), lastTick.getText(),
							anchor, anchor, lastTick.getAngle()));
	 ...
	 */
	public AxisState draw(Graphics2D g2, Rectangle2D area, Rectangle2D dataArea, RectangleEdge location,
			PlotRenderingInfo plotState) {

		AxisState ret = super.draw(g2, /* cursor, */area, /* dataArea, */ location, plotState);
		if (isAdvanceLineVisible()) {
			double xx = valueToJava2D(getRange().getUpperBound(), dataArea, location);
			Line2D mark = null;
			g2.setStroke(getAdvanceLineStroke());
			g2.setPaint(getAdvanceLinePaint());
			if (location == RectangleEdge.LEFT) {
				// TODO: the 0.0 values in the following need to be replaced
				// with the correct values - not sure what they are right now?
				mark = new Line2D.Double(
						// cursor, xx, cursor + dataArea.getWidth(), xx
						0.0, xx, 0.0 + dataArea.getWidth(), xx);
			} else if (location == RectangleEdge.RIGHT) {
				mark = new Line2D.Double(
						// cursor - dataArea.getWidth(), xx, cursor, xx
						0.0 - dataArea.getWidth(), xx, 0.0, xx);
			} else if (location == RectangleEdge.TOP) {
				mark = new Line2D.Double(
						// xx, cursor + dataArea.getHeight(), xx, cursor
						xx, 0.0 + dataArea.getHeight(), xx, 0.0);
			} else if (location == RectangleEdge.BOTTOM) {
				mark = new Line2D.Double(
						// xx, cursor, xx, cursor - dataArea.getHeight()
						xx, 0.0, xx, 0.0 - dataArea.getHeight());
			}
			g2.draw(mark);
		}
		return ret;
	...
	public boolean equals(Object obj) {
		...
		CyclicNumberAxis axis = (CyclicNumberAxis) obj;
		...
		if (!PaintUtilities.equal(this.advanceLinePaint, axis.advanceLinePaint)) {
			return false;
		}
		if (!ObjectUtilities.equal(this.advanceLineStroke, axis.advanceLineStroke)) {
			return false;
		}
