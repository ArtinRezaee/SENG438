...
public class LabelBlock extends AbstractContentBlock {
	...
	/**
	 * The text for the label - retained in case the label needs regenerating (for
	...
	protected ArrangeResult arrangeFR(Graphics2D g2, double fixedWidth, Range heightRange, ArrangeParams params) {
		...
		if (heightRange.contains(result.getHeight())) {
			...
			return arrangeFF(g2, fixedWidth, heightRange.constrain(result.getHeight()), params);
		}
	}
	...
	protected ArrangeResult arrangeRF(Graphics2D g2, Range widthRange, double fixedHeight, ArrangeParams params) {
		...
		if (widthRange.contains(result.getWidth())) {
			...
			return arrangeFF(g2, widthRange.constrain(result.getWidth()), fixedHeight, params);
		}
	}
	...
	protected ArrangeResult arrangeRR(Graphics2D g2, Range widthRange, Range heightRange, ArrangeParams params) {
		...
		if (widthRange.contains(result.getWidth()) && heightRange.contains(result.getHeight())) {
			return result;
		} else {
			return arrangeFF(g2, widthRange.constrain(result.getWidth()), heightRange.constrain(result.getHeight()),
					params);
		}
	}
	...
	protected ArrangeResult arrangeRN(Graphics2D g2, Range widthRange, ArrangeParams params) {
		...
		if (widthRange.contains(result.getWidth())) {
			...
			return arrangeFF(g2, widthRange.constrain(result.getWidth()), result.getHeight(), params);
		}
	}
	 ...
	 * @return The layout result (including *content* size).
	 */
	protected ArrangeResult arrangeNR(Graphics2D g2, Range heightRange, ArrangeParams params) {
		ArrangeResult result = arrangeNN(g2, params);
		if (heightRange.contains(result.getHeight())) {
			return result;
		} else {
			return arrangeFF(g2, result.getWidth(), heightRange.constrain(result.getHeight()), params);
		}
	}
