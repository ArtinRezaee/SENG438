...
public class AttributedLabelBlock extends AbstractContentBlock {
	 ...
	 * example, to change the block size).
	 */
	...
	public AttributedLabelBlock(AttributedString text, Font font) {
		this.text = text;
		this.label = ACIUtilities.createTextBlock(text.getIterator(), font, Color.black, HorizontalAlignment.LEFT);
		this.font = font;
	...
	public ArrangeResult arrange(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params) {
		...
		return new ArrangeResult(calculateTotalWidth(s.getWidth()), calculateTotalHeight(s.getHeight()), null);
	}
