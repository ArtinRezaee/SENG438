 ...
 * @author David Berry
 */
public abstract class Title extends AbstractContentBlock implements Block, Cloneable, Serializable {
	...
	/**
	 * Sets the background paint for the title and sends a {@link TitleChangeEvent}
	...
	public ArrangeResult arrange(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params) {
		...
		if (w == LengthConstraintType.NONE) {
			...
			if (h == LengthConstraintType.NONE) {
				...
				result = arrangeRR(g2, cc.getWidthRange(), cc.getHeightRange(), params);
			} else if (h == LengthConstraintType.FIXED) {
				result = arrangeRF(g2, cc.getWidthRange(), cc.getHeight(), params);
			}
		} else if (w == LengthConstraintType.FIXED) {
			if (h == LengthConstraintType.NONE) {
				result = arrangeFN(g2, cc.getWidth(), params);
			} else if (h == LengthConstraintType.RANGE) {
				result = arrangeFR(g2, cc.getWidth(), cc.getHeightRange(), params);
			} else if (h == LengthConstraintType.FIXED) {
		...
		result.setSize(calculateTotalWidth(result.getWidth()), calculateTotalHeight(result.getHeight()));
		return result;
	}
	 ...
	 * @return The layout result.
	 */
	protected abstract ArrangeResult arrangeNN(Graphics2D g2, ArrangeParams params);
	 ...
	 * @return The layout result.
	 */
	protected abstract ArrangeResult arrangeNF(Graphics2D g2, double fixedHeight, ArrangeParams params);
	 ...
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeRN(Graphics2D g2, Range widthRange, ArrangeParams params) {
		// TODO: review logging requirements
		ArrangeResult r = arrangeNN(g2, params);
		if (widthRange.contains(r.getWidth())) {
			return r;
		} else {
			return arrangeFF(g2, widthRange.constrain(r.getWidth()), r.getHeight(), params);
		}
	}

	/**
	 * Arranges the block with a range constraint on the width and height, returning
	 ...
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeRR(Graphics2D g2, Range widthRange, Range heightRange, ArrangeParams params) {

		// TODO: evaluate logging requirements
		ArrangeResult r = arrangeNN(g2, params);

		if (widthRange.contains(r.getWidth()) && heightRange.contains(r.getHeight())) {
			return r;
		} else {
			return arrangeFF(g2, widthRange.constrain(r.getWidth()), heightRange.constrain(r.getHeight()), params);
		}
	}

	/**
	 * Arranges the content of the title with a range constraint for the title width
	 * and a fixed height, and returns the size of the title content.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param widthRange
	 ...
	 *            the fixed height.
	 * @param params
	 *            the layout parameters (<code>null</code> permitted).
	 * 
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeRF(Graphics2D g2, Range widthRange, double fixedHeight, ArrangeParams params) {
		...
		if (widthRange.contains(r.getWidth())) {
			...
			return arrangeFF(g2, widthRange.constrain(r.getWidth()), fixedHeight, params);
		}
	}
	 ...
	 * @return The layout result.
	 */
	protected abstract ArrangeResult arrangeFN(Graphics2D g2, double fixedWidth, ArrangeParams params);

	/**
	 * Arranges the block with a fixed width and range constraint for the height,
	 ...
	 * @return The layout result.
	 */
	protected abstract ArrangeResult arrangeFF(Graphics2D g2, double fixedWidth, double fixedHeight,
