 ...
 * Contributor(s):   -;
 *
 * $Id: XYLineAndShapeRenderer.java,v 1.24 2005/12/12 14:11:32 mungady Exp $
 *
 * Changes:
 * --------
...
public class XYLineAndShapeRenderer extends AbstractXYItemRenderer
	...
	/**
	 * Draws the item (first pass). This method draws the lines connecting the
	 * items. Instead of drawing separate lines, a GeneralPath is constructed and
	 ...
	 * not <code>null</code> it will be populated with entity information.
	 *
	 * @param g2
	 *            the graphics device.
	 * @param dataArea
	...
	public LegendItem getLegendItem(int datasetIndex, int series) {
		...
		if (dataset != null) {
			if (getItemVisible(series, 0)) {
				...
				Paint fillPaint = (this.useFillPaint ? getSeriesFillPaint(series) : getSeriesPaint(series));
				boolean shapeOutlineVisible = this.drawOutlines;
				Paint outlinePaint = (this.useOutlinePaint ? getSeriesOutlinePaint(series) : getSeriesPaint(series));
				Stroke outlineStroke = getSeriesOutlineStroke(series);
				boolean lineVisible = getItemLineVisible(series, 0);
				Stroke lineStroke = getSeriesStroke(series);
				Paint linePaint = getSeriesPaint(series);
				if (attributedLabel != null) {
					result = new LegendItem(attributedLabel, description, toolTipText, urlText, shapeIsVisible, shape,
