...
public class LegendTitle extends Title implements Cloneable, PublicCloneable, Serializable {
	...
	/**
	 * Sets the padding that will be applied to the text for each item in the legend
	...
	protected Block createLegendItemBlock(LegendItem item) {
		...
		if (item.getAttributedLabel() != null) {
			labelBlock = new AttributedLabelBlock(item.getAttributedLabel(), this.itemFont);
		} else {
	...
	protected ArrangeResult arrangeFF(Graphics2D g2, double fixedWidth, double fixedHeight, ArrangeParams params) {
		...
		return container.arrange(g2, new RectangleConstraint(fixedWidth, fixedHeight), params);
	}
	...
	protected ArrangeResult arrangeFN(Graphics2D g2, double fixedWidth, ArrangeParams params) {
		...
		ArrangeResult naturalSize = container.arrange(g2, new RectangleConstraint(fixedWidth, null), params);
		if (h < 0.0) {
	...
	protected ArrangeResult arrangeNF(Graphics2D g2, double fixedHeight, ArrangeParams params) {
		...
		ArrangeResult naturalSize = container.arrange(g2, new RectangleConstraint(null, fixedHeight), params);
		if (w < 0.0) {
	...
	protected ArrangeResult arrangeNN(Graphics2D g2, ArrangeParams params) {
		...
		ArrangeResult naturalSize = container.arrange(g2, RectangleConstraint.NONE, params);
		if (w < 0.0) {
			w = naturalSize.getWidth();
		} else {
			w = trimToContentWidth(w);
