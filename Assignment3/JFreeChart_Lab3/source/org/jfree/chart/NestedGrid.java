...
public class NestedGrid extends AbstractContentBlock {
	...
	protected ArrangeResult arrangeNN(Graphics2D g2, ArrangeParams params) {
		...
		for (int c = 0; c < columns; c++) {
			for (int r = 0; r < rows; r++) {
				Block block = get(r, c);
				if (block != null) {
					ArrangeResult ar = block.arrange(g2, RectangleConstraint.NONE, params);
					Size2D size = ar.getSize();
					setRowHeight(r, Math.max(getRowHeight(r), size.getHeight()));
					setColumnWidth(c, Math.max(getColumnWidth(c), size.getWidth()));
				}
			}
		}
	...
	/**
	 * Arranges the blocks in the grid with fixed constraints for both the width and
	 * the height. First, the row heights and column widths are calculated without
	 * constraints. Next, all heights and widths are adjusted by the same proportion
	 * to achieve the desired fixed height.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param fixedWidth
	...
	protected ArrangeResult arrangeFF(Graphics2D g2, double fixedWidth, double fixedHeight, ArrangeParams params) {
		...
		for (int c = 0; c < getColumnCount(); c++) {
			...
			for (int r = 0; r < getRowCount(); r++) {
				...
				Block block = get(r, c);
				if (block != null) {
					RectangleConstraint cc = new RectangleConstraint(width, height);
	...
	/**
	 * Ensure that the grid contains sufficient capacity for the specified number of
	 * rows and columns.
	 * 
	 * @param rows
	 *            the row count.
	...
	/**
	 * Expands the grid structure, if necessary, to ensure that it can hold at least
	...
	/**
	 * Puts an element at the specified side of the element indicated by the key.
	 * This may require an expansion of the grid.
	 * 
	 * @param key
	 * @param edge
	 * @param block
		...
		for (int c = 0; c < columnCount; c++) {
			...
			for (int r = 0; r < rowCount; r++) {
				...
				if (element instanceof NestedGrid) {
					...
					subgrid.resizeAll(c * width, r * height, width / subgrid.getColumnCount(),
							height / subgrid.getRowCount(), g2);
