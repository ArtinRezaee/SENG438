...
public class StandardArrangement implements Arrangement, Serializable {
	 ...
	 * 
	 * @param chart
	 *            the chart that the arrangement is working on.
	 */
	...
	public ArrangeResult arrange(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
		...
		if (w == LengthConstraintType.NONE) {
			...
			if (h == LengthConstraintType.NONE) {
				...
				return arrangeFF(container, g2, constraint.getWidth(), constraint.getHeight(), params);
			} else if (h == LengthConstraintType.RANGE) {
				throw new RuntimeException("Not implemented.");
			}
	...
	protected ArrangeResult arrangeFF(BlockContainer container, Graphics2D g2, double fixedWidth, double fixedHeight,
		...
		Rectangle2D remaining = new Rectangle2D.Double(0.0, 0.0, fixedWidth, fixedHeight);
		...
		RectangleConstraint plotConstraint = new RectangleConstraint(remaining.getWidth(), remaining.getHeight());
	 ...
	 *            the title.
	 * @param remaining
	 *            the remaining area (updated by this method).
	 * @param g2
	 *            the graphics device.
	 * @param params
	 *            the layout parameters (<code>null</code> not permitted).
	 ...
	 */
	private ArrangeResult arrangeTitle(Title title, Rectangle2D remaining, Graphics2D g2, ArrangeParams params) {
		...
		if (position == RectangleEdge.TOP) {
			RectangleConstraint topConstraint = new RectangleConstraint(ww, new Range(0.0, hh));
			ArrangeResult r = title.arrange(g2, topConstraint, params);
			Size2D size = r.getSize();
			titleArea = createAlignedRectangle2D(size, remaining, title.getHorizontalAlignment(),
					VerticalAlignment.TOP);
			title.setBounds(titleArea);
			remaining.setRect(remaining.getX(), Math.min(remaining.getY() + size.height, remaining.getMaxY()),
					remaining.getWidth(), Math.max(remaining.getHeight() - size.height, 0));
			...
			title.setBounds(titleArea);
			remaining.setRect(remaining.getX(), remaining.getY(), remaining.getWidth(),
					remaining.getHeight() - size.height);
		} else if (position == RectangleEdge.RIGHT) {
			ArrangeResult r = title.arrange(g2, constraint, params);
			Size2D size = r.getSize();
			titleArea = createAlignedRectangle2D(size, remaining, HorizontalAlignment.RIGHT,
					title.getVerticalAlignment());
			title.setBounds(titleArea);
			remaining.setRect(remaining.getX(), remaining.getY(), remaining.getWidth() - size.width,
					remaining.getHeight());
		}
			...
			ArrangeResult r = title.arrange(g2, constraint, params);
			Size2D size = r.getSize();
			titleArea = createAlignedRectangle2D(size, remaining, HorizontalAlignment.LEFT,
					title.getVerticalAlignment());
			title.setBounds(titleArea);
			remaining.setRect(remaining.getX() + size.width, remaining.getY(), remaining.getWidth() - size.width,
					remaining.getHeight());
		} else {
