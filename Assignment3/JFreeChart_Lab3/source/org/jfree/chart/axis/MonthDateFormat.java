 ...
 * Contributor(s):   -;
 *
 * $Id: MonthDateFormat.java,v 1.2 2005/07/19 14:27:51 mungady Exp $
 *
 * Changes:
 * --------
...
public class MonthDateFormat extends DateFormat {
	 ...
	 * 
	 * @param locale
	 *            the locale used to obtain the month names (<code>null</code> not
	 *            permitted).
	 */
	 ...
	 *            the time zone (<code>null</code> not permitted).
	 * @param chars
	 *            the maximum number of characters to use from the month names (that
	 *            are obtained from the date symbols of the default locale). If this
	 ...
	 * 
	 * @param zone
	 *            the time zone used to extract the month and year from dates passed
	 *            to this formatter (<code>null</code> not permitted).
	 * @param locale
	 *            the locale used to determine the month names (<code>null</code>
	 *            not permitted).
	 * @param chars
	 *            the maximum number of characters to use from the month names, or
	 *            zero to indicate that the entire month name should be used.
	 * @param showYearForJan
	 *            a flag that controls whether or not the year is appended to the
	 *            symbol for the first month of each year.
	 * @param showYearForDec
	 *            a flag that controls whether or not the year is appended to the
	 *            symbol for the last month of each year.
	 */
	public MonthDateFormat(TimeZone zone, Locale locale, int chars, boolean showYearForJan, boolean showYearForDec) {
		this(zone, locale, chars, new boolean[] { showYearForJan, false, false, false, false, false, false, false,
				false, false, false, false, showYearForDec }, new SimpleDateFormat("yy"));
	}
	 ...
	 *            zero to indicate that the entire month name should be used.
	 * @param showYear
	 *            an array of flags that control whether or not the year is
	...
	public MonthDateFormat(TimeZone zone, Locale locale, int chars, boolean[] showYear, DateFormat yearFormatter) {
		...
		for (int i = 0; i < 12; i++) {
			if (chars > 0) {
				months[i] = monthsFromLocale[i].substring(0, Math.min(chars, monthsFromLocale[i].length()));
			} else {
