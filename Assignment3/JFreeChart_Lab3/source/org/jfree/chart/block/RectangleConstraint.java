 ...
 * Contributor(s):   -;
 *
 * $Id: RectangleConstraint.java,v 1.8 2005/07/19 14:26:39 mungady Exp $
 *
 * Changes:
 * --------
...
public class RectangleConstraint {
	 ...
	 * 
	 * @param w
	 *            the width range (<code>null</code> permitted).
	 * @param h
	 *            the height range (<code>null</code> permitted).
	 */
	...
	public RectangleConstraint(Range w, double h) {
		this(0.0, w, (w != null ? LengthConstraintType.RANGE : LengthConstraintType.NONE), h, null,
	...
	public RectangleConstraint(double w, Range h) {
		this(w, null, LengthConstraintType.FIXED, 0.0, h,
				(h != null ? LengthConstraintType.RANGE : LengthConstraintType.NONE));
	}
	 ...
	 * 
	 * @param w
	 *            the fixed or maximum width.
	 * @param widthRange
	 *            the width range.
	 * @param widthConstraintType
	 *            the width type.
	 * @param h
	 *            the fixed or maximum height.
	 * @param heightRange
	 *            the height range.
	 * @param heightConstraintType
	 *            the height type.
	 */
	...
	/**
	 * Returns a constraint that matches this one on the height attributes, but has
	 * no width constraint.
	 * 
	...
	public RectangleConstraint toUnconstrainedWidth() {
		if (this.widthConstraintType == LengthConstraintType.NONE) {
			...
			return new RectangleConstraint(this.width, this.widthRange, LengthConstraintType.NONE, this.height,
					this.heightRange, this.heightConstraintType);
		}
	}

	/**
	 * Returns a constraint that matches this one on the width attributes, but has
	...
	public RectangleConstraint toUnconstrainedHeight() {
		if (this.heightConstraintType == LengthConstraintType.NONE) {
			...
			return new RectangleConstraint(this.width, this.widthRange, this.widthConstraintType, 0.0, this.heightRange,
	...
	/**
	 * Returns a constraint that matches this one on the width attributes, but has a
	...
	public RectangleConstraint toFixedHeight(double height) {
		return new RectangleConstraint(this.width, this.widthRange, this.widthConstraintType, height, this.heightRange,
				LengthConstraintType.FIXED);
	}

	/**
	 * Returns a constraint that matches this one on the height attributes, but has
	 * a range width constraint.
	 * 
	 * @param range
	...
	public RectangleConstraint toRangeHeight(Range range) {
		...
		return new RectangleConstraint(this.width, this.widthRange, this.widthConstraintType, range.getUpperBound(),
				range, LengthConstraintType.RANGE);
	}

	/**
	 * Returns a string representation of this instance, mostly used for debugging
	 * purposes.
	 * 
	 * @return A string.
	 */
	public String toString() {
		return "RectangleConstraint[" + this.widthConstraintType.toString() + ": width=" + this.width + ", "
				+ this.heightConstraintType + ": height=" + this.height + "]";
	}
	...
	/**
	 * Calculates the constrained width.
	 * 
	 * @param baseWidth
	 *            the base width.
	 * 
	 ...
	 */
	public double calculateConstrainedWidth(double baseWidth) {
		if (this.widthConstraintType == LengthConstraintType.NONE) {
			return baseWidth;
		} else if (this.widthConstraintType == LengthConstraintType.FIXED) {
			return this.width;
		} else if (this.widthConstraintType == LengthConstraintType.RANGE) {
			return this.widthRange.constrain(baseWidth);
		} else {
			throw new RuntimeException("Unrecognised widthConstraintType: " + this.widthConstraintType);
		}
	}

	/**
	 * Calculates the constrained height.
	 * 
	 * @param baseHeight
	 *            the base height.
	 * 
	 ...
	 */
	public double calculateConstrainedHeight(double baseHeight) {
		if (this.heightConstraintType == LengthConstraintType.NONE) {
			return baseHeight;
		} else if (this.heightConstraintType == LengthConstraintType.FIXED) {
			return this.height;
		} else if (this.heightConstraintType == LengthConstraintType.RANGE) {
			return this.heightRange.constrain(baseHeight);
		} else {
			throw new RuntimeException("Unrecognised heightConstraintType: " + this.heightConstraintType);
		}
	}
