 ...
 */
public class BlockContainer extends AbstractContentBlock implements Block, Cloneable, PublicCloneable, Serializable {
	 ...
	 */
	public ArrangeResult arrangeOld(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params) {
	 ...
	 *            the constraint (<code>null</code> not permitted).
	 * @param params
	 *            arrangement parameters (<code>null</code> not permitted).
	 * 
	 * @return The block size (in Java2D units, never <code>null</code>).
	 */
	public ArrangeResult arrange(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params) {
		...
		if (w == LengthConstraintType.NONE) {
			...
			if (h == LengthConstraintType.NONE) {
				...
				contentSize = arrangeRR(g2, cc.getWidthRange(), cc.getHeightRange(), params);
			} else if (h == LengthConstraintType.FIXED) {
				contentSize = arrangeRF(g2, cc.getWidthRange(), cc.getHeight(), params);
			}
		} else if (w == LengthConstraintType.FIXED) {
			if (h == LengthConstraintType.NONE) {
				contentSize = arrangeFN(g2, cc.getWidth(), params);
			} else if (h == LengthConstraintType.RANGE) {
				contentSize = arrangeFR(g2, cc.getWidth(), cc.getHeightRange(), params);
			} else if (h == LengthConstraintType.FIXED) {
				contentSize = arrangeFF(g2, cc.getWidth(), cc.getHeight(), params);
			}
		}
	...
	/**
	 * Returns the size of the title content (excludes margin, border and padding)
	 * if there is no constraint. This is either the natural size of the text, or
	 * the block size if this has been specified manually.
	 * 
	 * @param g2
	 *            the graphics device.
	 * 
	...
	protected Size2D arrangeNN(Graphics2D g2, ArrangeParams params) {
		...
		Size2D naturalSize = this.arrangement.arrange(this, g2, RectangleConstraint.NONE, params).getSize();
		if (w < 0.0) {
	 ...
	 * @return The content size.
	 */
	protected Size2D arrangeRR(Graphics2D g2, Range widthRange, Range heightRange, ArrangeParams params) {
		Size2D naturalSize = arrangeNN(g2, params);
		if (!widthRange.contains(naturalSize.getWidth())) {
			return arrangeFR(g2, widthRange.getUpperBound(), heightRange, params);
		} else if (!heightRange.contains(naturalSize.getHeight())) {
			return arrangeRF(g2, widthRange, heightRange.getUpperBound(), params);
		}
		return naturalSize;
	}

	/**
	 * Arranges the container with the given constraints (a fixed width and a range
	 * of heights) and returns the calculated size.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param fixedWidth
	 ...
	 */
	protected Size2D arrangeFR(Graphics2D g2, double fixedWidth, Range heightRange, ArrangeParams params) {
		ArrangeResult r = this.arrangement.arrange(this, g2, new RectangleConstraint(fixedWidth, heightRange), params);
		return r.getSize();
	}

	protected Size2D arrangeFN(Graphics2D g2, double fixedWidth, ArrangeParams params) {
		return this.arrangement.arrange(this, g2, new RectangleConstraint(fixedWidth, null), params).getSize();
	}

	protected Size2D arrangeNF(Graphics2D g2, double fixedHeight, ArrangeParams params) {
		return this.arrangement.arrange(this, g2, new RectangleConstraint(null, fixedHeight), params).getSize();
	}

	protected Size2D arrangeRF(Graphics2D g2, Range widthRange, double fixedHeight, ArrangeParams params) {
		ArrangeResult r = this.arrangement.arrange(this, g2, new RectangleConstraint(widthRange, fixedHeight), params);
		return r.getSize();
	}
	 ...
	 * @return The content size.
	 */
	protected Size2D arrangeFF(Graphics2D g2, double fixedWidth, double fixedHeight, ArrangeParams params) {
		this.arrangement.arrange(this, g2, new RectangleConstraint(fixedWidth, fixedHeight), params);
	...
	public void draw(Graphics2D g2, Rectangle2D area) {
		arrange(g2, new RectangleConstraint(area.getWidth(), area.getHeight()), new ArrangeParams());
		draw(g2, area, null);
	}
