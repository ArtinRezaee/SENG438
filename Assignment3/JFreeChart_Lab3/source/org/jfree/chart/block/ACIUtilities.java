...
public class ACIUtilities {
	 ...
	 * 
	 * @return The text bounds (<code>null</code> if the <code>text</code> argument
	 *         is <code>null</code>).
	 */
	public static Rectangle2D getTextBounds(AttributedCharacterIterator text, Graphics2D g2) {
	 ...
	 */
	public static Rectangle2D drawAlignedString(AttributedCharacterIterator text, Graphics2D g2, float x, float y,
			TextAnchor anchor) {
	...
	/**
	 * A utility method that calculates the anchor offsets for a string. Normally,
	 * the (x, y) coordinate for drawing text is a point on the baseline at the left
	 * of the text string. If you add these offsets to (x, y) and draw the string,
	 * then the anchor point should coincide with the (x, y) point.
	 *
	 * @param g2
	 ...
	 *            the anchor point.
	 * @param textBounds
	 *            the text bounds (if not <code>null</code>, this object will be
	 *            updated by this method to match the string bounds).
	 * 
	 * @return The offsets.
	 */
	private static float[] deriveTextBoundsAnchorOffsets(final Graphics2D g2, AttributedCharacterIterator text,
			final TextAnchor anchor, final Rectangle2D textBounds) {
		...
		if (anchor == TextAnchor.TOP_CENTER || anchor == TextAnchor.CENTER || anchor == TextAnchor.BOTTOM_CENTER
				|| anchor == TextAnchor.BASELINE_CENTER || anchor == TextAnchor.HALF_ASCENT_CENTER) {
	 ...
	 * <P>
	 * A common rotation is -Math.PI/2 which draws text 'vertically' (with the top
	 * of the characters on the left).
	 *
	 * @param text
	 *            the text.
	 * @param g2
	 ...
	 *            the y-coordinate.
	 */
	public static void drawRotatedString(AttributedCharacterIterator text, Graphics2D g2, double angle, float x,
			float y) {
	 ...
	 *            the point about which the text is rotated.
	 */
	public static void drawRotatedString(AttributedCharacterIterator text, Graphics2D g2, float textX, float textY,
		...
		final TextLayout tl = new TextLayout(text, g2.getFontRenderContext());
	...
	/**
	 * Draws a string that is aligned by one anchor point and rotated about another
	 * anchor point.
	 *
	 * @param text
	 *            the text.
	 ...
	 */
	public static void drawRotatedString(AttributedCharacterIterator text, Graphics2D g2, float x, float y,
			TextAnchor textAnchor, double angle, float rotationX, float rotationY) {
	 ...
	 */
	public static void drawRotatedString(AttributedCharacterIterator text, Graphics2D g2, float x, float y,
			TextAnchor textAnchor, double angle, TextAnchor rotationAnchor) {
	...
	/**
	 * Returns a shape that represents the bounds of the string after the specified
	 ...
	 */
	public static Shape calculateRotatedStringBounds(AttributedCharacterIterator text, final Graphics2D g2,
			final float x, final float y, final TextAnchor textAnchor, final double angle,
	 ...
	 * @return The offsets.
	 */
	private static float[] deriveTextBoundsAnchorOffsets(Graphics2D g2, AttributedCharacterIterator text,
	 ...
	 * These offsets are relative to the text starting coordinate (BASELINE_LEFT).
	 *
	 * @param g2
	 *            the graphics device.
	 * @param text
	 ...
	 * @return The offsets.
	 */
	private static float[] deriveRotationAnchorOffsets(final Graphics2D g2, AttributedCharacterIterator text,
			final TextAnchor anchor) {
		...
		} else if (anchor == TextAnchor.TOP_CENTER || anchor == TextAnchor.CENTER || anchor == TextAnchor.BOTTOM_CENTER
				|| anchor == TextAnchor.BASELINE_CENTER || anchor == TextAnchor.HALF_ASCENT_CENTER) {
	 ...
	 */
	public static Shape calculateRotatedStringBounds(AttributedCharacterIterator text, Graphics2D g2, float textX,
	 ...
	 * @return A text line.
	 */
	public static TextLine createTextLine(AttributedCharacterIterator aci, Font defaultFont, Paint defaultPaint) {
		...
		while (ch != CharacterIterator.DONE) {
			...
			if (superscript.intValue() > 0) {
				font = font
						.deriveFont(AffineTransform.getScaleInstance(superscriptScaleFactor, superscriptScaleFactor));
				offset = offset * superscriptOffset;
			} else if (superscript.intValue() < 0) {
				font = font.deriveFont(AffineTransform.getScaleInstance(subscriptScaleFactor, subscriptScaleFactor));
				offset = offset * subscriptOffset;
			}
			result.addFragment(new TextFragment(sb.toString(), font, paint, offset));
	 ...
	 */
	public static TextBlock createTextBlock(AttributedCharacterIterator aci, Font defaultFont, Paint defaultPaint,
			...
			if (c == '\n') {
				...
				if (end > start) {
					TextLine line = createTextLine(
							new AttributedString((AttributedCharacterIterator) aci.clone(), start, end).getIterator(),
							defaultFont, defaultPaint);
					result.addLine(line);
					start = end + 1;
				}
			}
		...
		if (end > start) {
			TextLine line = createTextLine(new AttributedString(aci, start, end).getIterator(), defaultFont,
					defaultPaint);
			result.addLine(line);
			start = end + 1;
		}
	...
	/**
	 * Returns <code>true</code> if two {@link AttributedCharacterIterator}
	 * instances are the same or both <code>null</code>.
	 * 
	 * @param i1
	 *            character iterator 1 (<code>null</code> permitted).
	 * @param i2
	 *            character iterator 2 (<code>null</code> permitted).
	 * 
	 * @return A boolean.
	 */
	public static boolean equal(AttributedCharacterIterator i1, AttributedCharacterIterator i2) {
		...
		while (!done) {
			...
			if (c1 == AttributedCharacterIterator.DONE || c2 == AttributedCharacterIterator.DONE) {
				done = true;
			}
