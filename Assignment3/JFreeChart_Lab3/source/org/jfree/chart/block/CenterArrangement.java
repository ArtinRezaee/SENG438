...
/**
 * Arranges a single block in the center of its container. If the container has
 * more than one block, the first block is positioned in the center and the rest
 * are ignored. This class is immutable.
 */
public class CenterArrangement implements Arrangement, Serializable {
	...
	/**
	 * Adds a block to be managed by this instance. This method is usually called by
	 * the {@link BlockContainer}, you shouldn't need to call it directly.
	 * 
	 * @param block
	 *            the block.
	 * @param key
	...
	/**
	 * Calculates and sets the bounds of all the items in the specified container,
	 * subject to the given constraint. The <code>Graphics2D</code> can be used by
	 * some items (particularly items containing text) to calculate sizing
	 * parameters.
	 * 
	 * @param container
	...
	public ArrangeResult arrange(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
		...
		if (w == LengthConstraintType.NONE) {
			...
			if (h == LengthConstraintType.NONE) {
				return arrangeRN(container, g2, constraint.getWidthRange(), params);
			} else if (h == LengthConstraintType.FIXED) {
				return arrangeRF(container, g2, constraint.getWidthRange(), constraint.getHeight(), params);
			} else if (h == LengthConstraintType.RANGE) {
	...
	/**
	 * Arranges the first block in the container with a fixed width and no height
	 * constraint.
	 * 
	 * @param container
	 *            the container.
	 ...
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeFN(BlockContainer container, Graphics2D g2, double fixedWidth,
			ArrangeParams params) {
		...
		Rectangle2D bounds = new Rectangle2D.Double((fixedWidth - w) / 2.0, 0.0, w, h);
		b.setBounds(bounds);
	...
	/**
	 * Arranges the blocks in the container with a fixed with and a range constraint
	 * on the height.
	 * 
	 * @param container
	 *            the container.
	 ...
	 *            the fixed width.
	 * @param heightRange
	 *            the permitted height range.
	 * @param params
	 *            layout parameters (<code>null</code> not permitted).
	 * 
	 * @return The size following the arrangement.
	 */
	protected ArrangeResult arrangeFR(BlockContainer container, Graphics2D g2, double fixedWidth, Range heightRange,
			ArrangeParams params) {
	 ...
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeFF(BlockContainer container, Graphics2D g2, double fixedWidth, double fixedHeight,
			ArrangeParams params) {
		...
		Rectangle2D bounds = new Rectangle2D.Double(0.0, 0.0, fixedWidth, fixedHeight);
		b.setBounds(bounds);
	...
	/**
	 * Arranges the blocks with the overall width and height to fit within specified
	 * ranges.
	 * 
	 * @param container
	 *            the container.
	 ...
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeRR(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
			ArrangeParams params) {
		...
		if (constraint.getWidthRange().contains(s1.width)) {
			...
			return arrangeFR(container, g2, c.getWidth(), c.getHeightRange(), params);
		}
	}

	/**
	 * Arranges the blocks in the container with a range constraint on the width and
	 * a fixed height.
	 * 
	 * @param container
	 *            the container.
	 ...
	 * @return The size following the arrangement.
	 */
	protected ArrangeResult arrangeRF(BlockContainer container, Graphics2D g2, Range widthRange, double fixedHeight,
			ArrangeParams params) {
	...
	/**
	 * Arranges the block with a range constraint on the width, and no constraint on
	 * the height.
	 * 
	 * @param container
	 *            the container.
	 ...
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeRN(BlockContainer container, Graphics2D g2, Range widthRange, ArrangeParams params) {
	 ...
	 * This puts all blocks into a single row.
	 * 
	 * @param container
	 *            the container.
	 * @param g2
	 ...
	 *            the fixed height.
	 * @param params
	 *            layout parameters (<code>null</code> permitted).
	 * 
	 * @return The layout result.
	 */
	protected ArrangeResult arrangeNF(BlockContainer container, Graphics2D g2, double fixedHeight,
			ArrangeParams params) {
