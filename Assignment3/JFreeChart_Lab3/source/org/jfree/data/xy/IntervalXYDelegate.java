 ...
 *
 * $Id: IntervalXYDelegate.java,v 1.11 2005/10/06 13:26:55 mungady Exp $
 *
...
public class IntervalXYDelegate implements DatasetChangeListener, DomainInfo, Serializable, Cloneable, PublicCloneable {
	...
	/**
	 * A value between 0.0 and 1.0 that indicates the position of the x-value within
	 * the interval.
	 */
	...
	/**
	 * Returns <code>true</code> if the interval width is automatically calculated,
	...
	/**
	 * Sets the interval position factor. This controls how the interval is aligned
	 * to the x-value. For a value of 0.5, the interval is aligned with the x-value
	 * in the center. For a value of 0.0, the interval is aligned with the x-value
	 * at the lower end of the interval, and for a value of 1.0, the interval is
	 * aligned with the x-value at the upper end of the interval.
	 * 
	 * Note that changing the interval position factor amounts to changing the data
	 * values represented by the dataset. Therefore, the dataset that is using this
	 * delegate is responsible for generating the appropriate
	 * {@link DatasetChangeEvent}.
	 * 
	 * @param d
	 *            the new interval position factor (in the range <code>0.0</code> to
	 ...
	 * calculated interval width or the manually specified interval width, depending
	 * on the {@link #isAutoWidth()} result.
	 * 
	...
	public double getEndXValue(int series, int item) {
		return dataset.getXValue(series, item) + (1.0 - getIntervalPositionFactor()) * getIntervalWidth();
	}
