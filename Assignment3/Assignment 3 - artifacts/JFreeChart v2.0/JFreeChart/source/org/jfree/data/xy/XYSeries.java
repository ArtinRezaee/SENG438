 ...
 *                   Michel Santos;
 *
 * $Id: XYSeries.java,v 1.14 2006/01/11 15:27:41 mungady Exp $
 *
 * Changes
 * -------
...
public class XYSeries extends Series implements Cloneable, Serializable {
	...
	/**
	 * Returns the maximum number of items that will be retained in the series. The
	 * default value is <code>Integer.MAX_VALUE</code>.
	 *
	...
	/**
	 * Sets the maximum number of items that will be retained in the series. If you
	 * add a new item to the series such that the number of items will exceed the
	 * maximum item count, then the first element in the series is automatically
	 * removed, ensuring that the maximum item count is not exceeded.
	 * <p>
	 * Typically this value is set before the series is populated with data, but if
	 * it is applied later, it may cause some items to be removed from the series
	 * (in which case a {@link SeriesChangeEvent} will be sent to all registered
	 * listeners.
	 *
	 * @param maximum
	 *            the maximum number of items for the series.
	...
	/**
	 * Adds a data item to the series and sends a {@link SeriesChangeEvent} to all
	 * registered listeners. The unusual pairing of parameter types is to make it
	 * easier to add <code>null</code> y-values.
	 *
	 * @param x
	 *            the x value.
	 * @param y
	...
	/**
	 * Adds a data item to the series and, if requested, sends a
	 * {@link SeriesChangeEvent} to all registered listeners. The unusual pairing of
	 * parameter types is to make it easier to add null y-values.
	 *
	 * @param x
	 *            the x value.
	 * @param y
	...
	/**
	 * Removes the item at the specified index and sends a {@link SeriesChangeEvent}
	 ...
	 * 
	 * @deprecated Renamed updateByIndex(int, Number) to avoid confusion with the
	...
	/**
	 * Returns the index of the item with the specified x-value, or a negative index
	 * if the series does not contain an item with that x-value. Be aware that for
	 * an unsorted series, the index is found by iterating through all items in the
	 * series.
	 * 
	 * @param x
	 *            the x-value (<code>null</code> not permitted).
