 ...
 * Contributor(s):   -;
 *
 * $Id: SimpleHistogramDataset.java,v 1.7 2005/05/20 15:08:37 mungady Exp $
 *
 * Changes
 * -------
...
public class SimpleHistogramDataset extends AbstractIntervalXYDataset
	...
	/**
	 * Returns a flag that controls whether or not the bin count is divided by the
	 * bin size in the {@link #getXValue(int, int)} method.
	 * 
	 * @return A boolean.
	 */
	...
	/**
	 * Sets the flag that controls whether or not the bin count is divided by the
	 * bin size in the {@link #getXValue(int, int)} method.
	 * 
	 * @param adjust
	 *            the flag.
	...
	/**
	 * Adds a bin to the dataset. An exception is thrown if the bin overlaps with
	 * any existing bin in the dataset.
	 * 
	 * @param bin
	 *            the bin (<code>null</code> not permitted).
	 */
	...
	/**
	 * Adds an observation to the dataset (by incrementing the item count for the
	 * appropriate bin). A runtime exception is thrown if the value does not fit
	 * into any bin.
	 * 
	 * @param value
	 *            the value.
	 ...
	 *            the value.
	 * @param notify
	 *            send {@link DatasetChangeEvent} to listeners?
	 */
