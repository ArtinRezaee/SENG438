...
public strictfp class Range implements Serializable {
	 ...
	 * @param upper
	 *            the upper bound (should be >= lower bound).
	 * 
	 ...
	 * 
	 * @param value
	 *            the value to find the closest in-range value of.
	 * 
	 * @return The constrained value. If value is within the range, will return the
	 *         input value.
	 */
	...
	/**
	 * Creates a new range by adding margins to an existing range. For example:
	 * expand(new Range(2, 6), 0.25, 0.5) returns a range from 1 to 8.
	 * 
	 * @param range
	 ...
	 *            a flag that determines whether or not the bounds of the range are
	...
	/**
	 * Returns the given <code>value</code> adjusted by <code>delta</code> but with
	 * a check to prevent the result from crossing <code>0.0</code>.
	 * 
	 * @param value
	 *            the value.
	 * @param delta
