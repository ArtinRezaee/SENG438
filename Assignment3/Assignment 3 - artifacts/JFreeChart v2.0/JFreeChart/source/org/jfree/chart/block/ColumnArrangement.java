...
public class ColumnArrangement implements Arrangement, Serializable {
	...
	protected ArrangeResult arrangeFN(BlockContainer container, Graphics2D g2, double fixedWidth,
		...
		// the fixedWidth
		RectangleConstraint constraint = new RectangleConstraint(new Range(0.0, fixedWidth), null);
		for (int i = 0; i < blockCount; i++) {
			Block b = (Block) blocks.get(i);
			ArrangeResult r = b.arrange(g2, constraint, params);
			b.setBounds(new Rectangle2D.Double(0.0, totalHeight, r.getWidth(), r.getHeight()));
	 ...
	 * @return The container size after the arrangement.
	 */
	protected ArrangeResult arrangeNF(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
			ArrangeParams params) {
		...
		for (int i = 0; i < blocks.size(); i++) {
			...
			ArrangeResult r = block.arrange(g2, RectangleConstraint.NONE, params);
			if (y + r.getHeight() <= height) {
				itemsInColumn.add(block);
				block.setBounds(new Rectangle2D.Double(x, y, r.getWidth(), r.getHeight()));
				...
				if (itemsInColumn.isEmpty()) {
					...
					block.setBounds(new Rectangle2D.Double(x, y, r.getWidth(), Math.min(r.getHeight(), height - y)));
					y = 0.0;
					...
					y = 0.0;
					maxWidth = r.getWidth();
					block.setBounds(new Rectangle2D.Double(x, y, r.getWidth(), Math.min(r.getHeight(), height)));
	...
	/**
	 * Arranges the blocks in the container using a fixed height and a range for the
	 * width.
	 * 
	 * @param container
	 *            the container.
	 ...
	 */
	protected ArrangeResult arrangeRF(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
			ArrangeParams params) {
