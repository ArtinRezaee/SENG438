 ...
 *                   Eduardo Ramalho;
 *
 * $Id: XYPlot.java,v 1.50 2006/01/26 16:48:36 mungady Exp $
 *
...
public class XYPlot extends Plot
		implements Block, ValueAxisPlot, Zoomable, RendererChangeListener, Cloneable, PublicCloneable, Serializable {
	 ...
	 * contains no entry for a dataset, it is assumed to map to the primary domain
	 * axis (index = 0).
	 */
	...
	/**
	 * A flag that controls whether or not the zero baseline is visible on the main
	 * range axis.
	 */
	...
	/**
	 * The order of the series rendering (REVERSE draws the primary series last so
	 * that it appears to be on top).
	 */
	...
	/**
	 * Sets a domain axis and, if requested, sends a {@link PlotChangeEvent} to all
	 * registered listeners.
	 *
	 * @param index
	 *            the axis index.
	...
	/**
	 * Sets the domain axes for this plot and sends a {@link PlotChangeEvent} to all
	 * registered listeners.
	 * 
	 * @param axes
	 *            the axes.
	...
	/**
	 * Returns the edge for the specified axis.
	 * 
	 * @param axis
	 *            the axis.
	 * 
	 * @return The edge.
	 */
	public RectangleEdge getAxisEdge(ValueAxis axis) {
		RectangleEdge result = null;
		int index = this.domainAxes.indexOf(axis);
		if (index >= 0) {
			result = getDomainAxisEdge(index);
		} else {
			index = this.rangeAxes.indexOf(axis);
			if (index >= 0) {
				result = getRangeAxisEdge(index);
			}
		}
		return result;
	}
	...
	/**
	 * Sets a range axis and, if requested, sends a {@link PlotChangeEvent} to all
	 * registered listeners.
	 *
	 * @param index
	 *            the axis index.
	...
	/**
	 * Sets the range axes for this plot and sends a {@link PlotChangeEvent} to all
	 * registered listeners.
	 * 
	 * @param axes
	 *            the axes.
	...
	/**
	 * Maps a dataset to a particular domain axis. All data will be plotted against
	...
	/**
	 * Maps a dataset to a particular range axis. All data will be plotted against
	 * axis zero by default, no mapping is required for this case.
	 *
	 * @param index
	 *            the dataset index (zero-based).
	...
	/**
	 * Sets the renderers for this plot and sends a {@link PlotChangeEvent} to all
	 * registered listeners.
	 * 
	 * @param renderers
	 *            the renderers.
	...
	/**
	 * Sets the series order and sends a {@link PlotChangeEvent} to all registered
	 * listeners. By default, the plot renders the primary series last (so that the
	 * primary series appears to be on top). You can reverse this if you want to.
	 *
	 * @param order
	 *            the rendering order (<code>null</code> not permitted).
	 */
	...
	/**
	 * Returns the renderer for the specified dataset. The code first determines the
	 * index of the dataset, then checks if there is a renderer with the same index
	 * (if not, the method returns renderer(0).
	 *
	 * @param dataset
	 *            the dataset (<code>null</code> permitted).
	 *
	...
	/**
	 * Sets the stroke for the grid lines plotted against the range axis, and sends
	...
	/**
	 * Returns a flag that controls whether or not a zero baseline is displayed for
	 * the range axis.
	 *
	 * @return A boolean.
	 */
	...
	/**
	 * Sets the flag that controls whether or not the zero baseline is displayed for
	 * the range axis, and sends a {@link PlotChangeEvent} to all registered
	 * listeners.
	 *
	 * @param visible
	 *            the flag.
	...
	/**
	 * Sets the paint for the zero baseline plotted against the range axis and sends
	 * a {@link PlotChangeEvent} to all registered listeners.
	 *
	 * @param paint
	 *            the paint (<code>null</code> permitted).
	...
	public ArrangeResult arrange(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params) {
		...
		if (width < 0.0) {
			// TODO: if there is a width calculator, use it
			width = 300.0;
		}

		double height = getDefaultHeight();
		if (height < 0.0 && params != null) {
			BlockHeightCalculator bhc = params.findHeightCalculator(this);
			if (bhc != null) {
				height = bhc.calculateHeight(this);
				// TODO: this calculated height is for the content? Or the
				// bounds? needs to be the bounds.
			} else {
		...
		double w = constraint.calculateConstrainedWidth(width);
		double h = constraint.calculateConstrainedHeight(height);
		if (result != null) {
			result.setSize(w, h);
		} else {
	 ...
	 *            the area.
	 * @param params
	 *            optional drawing parameters (ignored here).
	 * 
	 * @return Optional drawing results (<code>null</code> in this case).
	 */
	...
	public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor, PlotState parentState, PlotRenderingInfo info) {
		...
		if (this.domainGridlinesVisible) {
			drawDomainGridlines(g2, dataArea);
		}
		if (this.rangeGridlinesVisible) {
			drawRangeGridlines(g2, dataArea);
		}
	...
	protected void drawDomainGridlines(Graphics2D g2, Rectangle2D dataArea) {
		...
		List ticks = getDomainAxis().getTicks();
	...
	protected void drawRangeGridlines(Graphics2D g2, Rectangle2D dataArea) {
		List ticks = getRangeAxis().getTicks();
		...
		if (axis != null) {
			...
			while (iterator.hasNext()) {
				...
				if (tick.getValue() != 0.0 || !isRangeZeroBaselineVisible()) {
					getRenderer().drawRangeLine(g2, this, getRangeAxis(), dataArea, tick.getValue(), gridPaint,
							gridStroke);
				}
			}
		}
		if (isRangeZeroBaselineVisible() && axis.getRange().contains(0.0)) {
			getRenderer().drawRangeLine(g2, this, axis, dataArea, 0.0, getRangeZeroBaselinePaint(),
	 ...
	 *            the data area.
	 */
	protected void drawZeroRangeBaseline(Graphics2D g2, Rectangle2D dataArea) {

		if (isRangeZeroBaselineVisible()) {
			getRenderer().drawRangeLine(g2, this, getRangeAxis(), dataArea, 0.0, this.rangeZeroBaselinePaint,
