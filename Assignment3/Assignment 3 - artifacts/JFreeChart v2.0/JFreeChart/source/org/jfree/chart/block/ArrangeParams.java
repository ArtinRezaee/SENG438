...
/**
 * A class for holding parameters to pass to the arrange() method in the Block
 * class.
 */
public class ArrangeParams implements BlockSizeCalculatorList {
	...
	/**
	 * A flag that controls whether or not log output should be appended to the
	 * results object.
	 */
	private boolean log;
	 ...
	 * 
	 * @param result
	 *            the result (<code>null</code> permitted).
	 */
	...
	public BlockWidthCalculator findWidthCalculator(Block block) {
		...
		for (int i = 0; i < this.widthCalculators.size(); i++) {
			BlockWidthCalculator bwc = (BlockWidthCalculator) this.widthCalculators.get(i);
	...
	public BlockHeightCalculator findHeightCalculator(Block block) {
		...
		for (int i = 0; i < this.heightCalculators.size(); i++) {
			BlockHeightCalculator bhc = (BlockHeightCalculator) this.heightCalculators.get(i);
