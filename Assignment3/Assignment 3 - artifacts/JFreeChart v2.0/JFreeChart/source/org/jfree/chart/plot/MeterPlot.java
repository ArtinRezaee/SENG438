...
public class MeterPlot extends Plot implements Serializable, Cloneable {
	...
	/**
	 * A (possibly empty) list of the {@link MeterInterval}s to be highlighted on
	 * the dial.
	 */
	private List intervals;

	/**
	 * Creates a new plot with a default range of <code>0</code> to <code>100</code>
	...
	/**
	 * Returns the meter angle in degrees. This defines, in part, the shape of the
	 * dial. The default is 270 degrees.
	 *
	 ...
	 * 
	 * @param range
	 *            the range (<code>null</code> not permitted and zero-length ranges
	 *            not permitted).
	 */
	...
	/**
	 * Sets the paint used to display the needle and sends a {@link PlotChangeEvent}
	 ...
	 * 
	 * @param interval
	 *            the interval (<code>null</code> not permitted).
	 */
	...
	/**
	 * Clears the intervals for the plot and sends a {@link PlotChangeEvent} to all
	 * registered listeners.
	 */
	...
	public LegendItemCollection getLegendItems() {
		...
		while (iterator.hasNext()) {
			...
			LegendItem item = new LegendItem(mi.getLabel(), mi.getLabel(), null, null,
	 ...
	 */
	protected void fillArc(Graphics2D g2, Rectangle2D area, double minValue, double maxValue, Paint paint,
			boolean dial) {
		if (paint == null) {
			throw new IllegalArgumentException("Null 'paint' argument");
		}
	...
	/**
	 * Returns an independent copy (clone) of the plot. The dataset is NOT cloned -
	 * both the original and the clone will have a reference to the same dataset.
	 * 
	 * @return A clone.
	 * 
