...
public class SymbolAxis extends NumberAxis implements Serializable {
	 ...
	 */
	public AxisState draw(Graphics2D g2, Rectangle2D area, RectangleEdge edge, PlotRenderingInfo plotState) {

		AxisState info = new AxisState(0.0);
		if (isVisible()) {
			info = super.draw(g2, area, edge, plotState);
		}
		if (this.gridBandsVisible) {
			drawGridBands(g2, area, area, edge, info.getTicks());
		}
		return info;
	...
	protected void drawGridBandsHorizontal(Graphics2D g2, Rectangle2D plotArea, Rectangle2D dataArea,
		...
		while (iterator.hasNext()) {
			...
			band = new Rectangle2D.Double(xx1, yy, xx2 - xx1, dataArea.getMaxY() - yy);
			g2.fill(band);
	...
	protected void drawGridBandsVertical(Graphics2D g2, Rectangle2D drawArea, Rectangle2D plotArea,
		...
		while (iterator.hasNext()) {
			...
			band = new Rectangle2D.Double(xx, yy1, plotArea.getMaxX() - xx, yy2 - yy1);
			g2.fill(band);
			currentGridBandIsDark = !currentGridBandIsDark;
		}
		g2.setPaintMode();
