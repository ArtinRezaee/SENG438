 ...
 * {@link ContentBlock} interface.
 */
public abstract class AbstractContentBlock implements ContentBlock, Serializable {
	...
	/**
	 * The default width of the block (may be overridden if there are constraints in
	 * sizing).
	 */
	private double defaultWidth;

	/**
	 * The default height of the block (may be overridden if there are constraints
	 * in sizing).
	 */
	...
	/**
	 * Returns the default width of the block, if this is known in advance. The
	 * actual width of the block may be overridden if layout constraints make this
	 * necessary.
	 * 
	...
	/**
	 * Returns the default height of the block, if this is known in advance. The
	 * actual height of the block may be overridden if layout constraints make this
	 * necessary.
	 * 
	...
	/**
	 * Sets the padding (use {@link RectangleInsets#ZERO_INSETS} for no padding).
	 * 
	 * @param padding
	...
	/**
	 * Returns the background paint for the block. This is used to fill the entire
	 * background area for the block. To fill only the area inside the block's
	 * border, use {@link #setInteriorBackgroundPaint(Paint)}. If the paint is
	 * <code>null</code>, the block will appear to be transparent.
	 * 
	...
	/**
	 * Returns the interior background paint for the block. This is used to fill the
	 * background area inside the block's border (which we call the interior). To
	 * fill the entire background area (that is, including the margin area), use
	 * {@link #setBackgroundPaint(Paint)}. If the paint is <code>null</code>, the
	 * interior of the block will appear to be transparent.
	 * 
	 * @return The background paint (possibly <code>null</code>).
	 */
	 ...
	 */
	public abstract ArrangeResult arrange(Graphics2D g2, RectangleConstraint constraint, ArrangeParams params);
	...
	/**
	 * Returns a constraint for the content of this block that will result in the
	 * bounds of the block matching the specified constraint.
	 * 
	 * @param c
	...
	public boolean equals(Object obj) {
		...
		if (!PaintUtilities.equal(this.interiorBackgroundPaint, that.interiorBackgroundPaint)) {
			return false;
		}
		return true;
	}
