...
public class BorderArrangement implements Arrangement, Serializable {
	...
	public ArrangeResult arrange(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
		...
		if (w == LengthConstraintType.NONE) {
			...
			if (h == LengthConstraintType.NONE) {
				...
				result = arrangeFF(container, g2, constraint.getWidth(), constraint.getHeight(), params);
			} else if (h == LengthConstraintType.RANGE) {
			...
			if (h == LengthConstraintType.NONE) {
				...
				result = arrangeRR(container, g2, constraint.getWidthRange(), constraint.getHeightRange(), params);
			}
		}
	...
	protected ArrangeResult arrangeFR(BlockContainer container, Graphics2D g2, RectangleConstraint constraint,
			ArrangeParams params) {
		ArrangeResult r1 = arrangeFN(container, g2, constraint.getWidth(), params);
	 ...
	 */
	protected ArrangeResult arrangeFN(BlockContainer container, Graphics2D g2, double width, ArrangeParams params) {
		...
		RectangleConstraint c2 = new RectangleConstraint(0.0, new Range(0.0, width), LengthConstraintType.RANGE, 0.0,
		...
		if (this.centerBlock != null) {
			RectangleConstraint c4 = new RectangleConstraint(width - w[2] - w[3], null, LengthConstraintType.FIXED, 0.0,
					null, LengthConstraintType.NONE);
	 ...
	 */
	protected ArrangeResult arrangeRR(BlockContainer container, Graphics2D g2, Range widthRange, Range heightRange,
			ArrangeParams params) {
		double[] w = new double[5];
		double[] h = new double[5];
		if (this.topBlock != null) {
	...
	/**
	 * Arranges the items within a container to achieve the fixed width and height.
	 * 
	 * @param container
	 *            the container.
	 * @param g2
	 ...
	 * @return The container size after the arrangement.
	 */
	protected ArrangeResult arrangeFF(BlockContainer container, Graphics2D g2, double width, double height,
			ArrangeParams params) {
		double[] w = new double[5];
		double[] h = new double[5];
		w[0] = width;
		if (this.topBlock != null) {
			RectangleConstraint c1 = new RectangleConstraint(width, new Range(0.0, height));
			ArrangeResult r = this.topBlock.arrange(g2, c1, params);
			Size2D size = r.getSize();
			h[0] = size.height;
		}
		w[1] = w[0];
		if (this.bottomBlock != null) {
			RectangleConstraint c2 = new RectangleConstraint(w[0], new Range(0.0, height - h[0]));
		...
		h[2] = height - h[1] - h[0];
		if (this.leftBlock != null) {
			RectangleConstraint c3 = new RectangleConstraint(new Range(0.0, width), h[2]);
		...
		h[3] = h[2];
		if (this.rightBlock != null) {
			RectangleConstraint c4 = new RectangleConstraint(new Range(0.0, width - w[2]), h[2]);
	...
	/**
	 * Tests this <code>BorderArrangement</code> for equality with an arbitrary
