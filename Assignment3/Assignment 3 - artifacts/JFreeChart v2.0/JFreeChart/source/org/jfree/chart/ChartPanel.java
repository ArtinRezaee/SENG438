 ...
 *                   Onno vd Akker;
 *
 * $Id: ChartPanel.java,v 1.23 2005/10/14 09:45:35 mungady Exp $
 *
 ...
 */
public class ChartPanel extends JPanel implements ChartChangeListener, ChartProgressListener, ActionListener,
	...
	/**
	 * Sets the scale factor. This method is provided for the use of subclasses, but
	 * shouldn't be required for end users.
	 * 
	 * @param factor
	 *            the scale factor.
	 */
	protected void setScaleX(double factor) {
		this.scaleX = factor;
	}
	...
	/**
	 * Sets the Y scale factor. This method is provided for subclasses but shouldn't
	 * be required by end users.
	 * 
	 * @param factor
	 *            the scale factor.
	 */
	protected void setScaleY(double factor) {
		this.scaleY = factor;
	}
	...
	/**
	 * Switches the display of tooltips for the panel on or off. Note that tooltips
	 * can only be displayed if the chart has been configured to generate tooltip
	 * items.
	 *
	 * @param flag
	 *            <code>true</code> to enable tooltips, <code>false</code> to
	 *            disable tooltips.
	 */
	...
	/**
	 * Returns the flag that controls whether or not the offscreen buffer needs to
	 * be refreshed.
	 * 
	 * @return A boolean.
	 */
	...
	public void paintComponent(Graphics g) {
		...
		// );
		// getChart().arrange(g2, constraint);
		...
		if (this.useBuffer) {
			...
			if (this.refreshBuffer) {
				...
				RectangleConstraint constraint = new RectangleConstraint(this.chartBufferWidth, this.chartBufferHeight);
				getChart().arrange(g2, constraint, null);
		...
		// or redrawing the chart every time...
			...
			RectangleConstraint constraint = new RectangleConstraint(drawWidth, drawHeight);
			getChart().arrange(g2, constraint, new ArrangeParams());
			getChart().draw(g2, chartArea, getAnchor(), getChartRenderingInfo());
			g2.setTransform(saved);
	...
	/**
	 * Handles a 'mouse released' event. On Windows, we need to check if this is a
	 * popup trigger, but only if we haven't already been tracking a zoom rectangle.
	 *
	 * @param e
