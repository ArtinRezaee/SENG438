 ...
 * Contributor(s):   -;
 *
 * $Id: MultiplePiePlot.java,v 1.17 2006/04/06 16:22:26 mungady Exp $
 *
 ...
 */
public class MultiplePiePlot extends Plot implements Block, Cloneable, Serializable {
	...
	/**
	 * Returns the key for aggregated items in the pie plots, if there are any. The
	...
	/**
	 * Sets the key for aggregated items in the pie plots. You must ensure that this
	 * doesn't clash with any keys in the dataset.
	 * 
	 * @param key
	...
	/**
	 * Returns the paint used to draw the pie section representing the aggregated
	 * items. The default value is <code>Color.lightGray</code>.
	 * 
	 * @return The paint.
	 * 
	 * @since 1.0.2
	...
	/**
	 * Sets the paint used to draw the pie section representing the aggregated items
	...
	/**
	 * For each key in the dataset, check the <code>sectionPaints</code> cache to
	 * see if a paint is associated with that key and, if not, fetch one from the
	 * drawing supplier. These colors are cached so that the legend and all the
	...
	private void prefetchSectionPaints() {
		...
		if (this.dataExtractOrder == TableOrder.BY_ROW) {
			...
			for (int c = 0; c < this.dataset.getColumnCount(); c++) {
				...
				Paint p = (Paint) this.sectionPaints.get(key);
				if (p == null) {
					this.sectionPaints.put(key, this.getDrawingSupplier().getNextPaint());
				}
			}
		} else {
		...
		if (this.dataset != null) {
			...
			if (keys != null) {
				...
				while (iterator.hasNext()) {
					...
					LegendItem item = new LegendItem(label, description, null, null, Plot.DEFAULT_LEGEND_ITEM_CIRCLE,
							paint, new BasicStroke(1.0f), paint);
			...
			if (this.limit > 0.0) {
				result.add(new LegendItem(this.aggregatedItemsKey.toString(), this.aggregatedItemsKey.toString(), null,
						null, Plot.DEFAULT_LEGEND_ITEM_CIRCLE, this.aggregatedItemsPaint, new BasicStroke(1.0f),
	 ...
	 * dataset is not considered in the equality test.
	 * 
	 * @param obj
