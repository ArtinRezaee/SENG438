 ...
 *
 * $Id: NumberAxis.java,v 1.18 2006/02/10 16:14:17 mungady Exp $
 *
 ...
 */
public class NumberAxis extends ValueAxis implements Block, Cloneable, Serializable {
	...
	/**
	 * The range type (can be used to force the axis to display only positive values
	 ...
	 * <p>
	 * Note: if the <code>autoTickUnitSelection</code> flag is <code>true</code> the
	 * tick unit may be changed while the axis is being drawn, so in that case the
	 ...
	 * @param edge
	 *            the location of the axis relative to the plot (<code>null</code>
	 *            not permitted).
	 * @param plotState
	 *            collects information about the plot (<code>null</code> permitted).
	 * 
	 * @return The axis state (<code>null</code>).
	 */
	public AxisState draw(Graphics2D g2, Rectangle2D area, // for the axis
			RectangleEdge edge, PlotRenderingInfo plotState) {

		// our API here reflects the old JFreeChart approach
		// we should only use:
		// g2 = graphics device;
		// area = region for drawing AXIS
		// edge = the axis location
		if (!isVisible()) {
			return null;
		}
		drawTicks(g2, area, edge);
	 ...
	 *            the graphics device.
	 * @param area
	 *            the area within which the axis is drawn.
	 * @param location
	 *            the location of the axis relative to the plot.
	 */
	protected void drawTicks(Graphics2D g2, Rectangle2D area, RectangleEdge location) {
		...
		while (iterator.hasNext()) {
			...
			if (isTickLabelsVisible()) {
				float[] anchorPoint = calculateAnchorPoint(tick, area, location);
				// g2.setPaint(Color.red);
				// g2.drawLine((int) anchorPoint[0], (int) anchorPoint[1], (int) anchorPoint[0],
				// (int) anchorPoint[1]);
			...
			if (isTickMarksVisible()) {
				float xx = (float) valueToJava2D(tick.getValue(), area, location);
				Line2D mark = null;
				g2.setStroke(getTickMarkStroke());
				g2.setPaint(getTickMarkPaint());
				if (location == RectangleEdge.LEFT) {
					double maxX = area.getMaxX();
					mark = new Line2D.Double(maxX - ol, xx, maxX + il, xx);
				} else if (location == RectangleEdge.RIGHT) {
					double minX = area.getMinX();
					mark = new Line2D.Double(minX + ol, xx, minX - il, xx);
				} else if (location == RectangleEdge.TOP) {
					double maxY = area.getMaxY();
					mark = new Line2D.Double(xx, maxY - ol, xx, maxY + il);
				} else if (location == RectangleEdge.BOTTOM) {
					double minY = area.getMinY();
					mark = new Line2D.Double(xx, minY + ol, xx, minY - il);
				}
				g2.draw(mark);
			}
		}
	 ...
	 * {@link TickUnits} and then pass it to the <code>setStandardTickUnits()</code>
	 * method.
	 *
	 * @param locale
	 *            the locale.
	...
	/**
	 * Creates a list of ticks (including values and formatted labels) for the axis.
	 *
	 * @param g2
	 *            the graphics device.
	 * @param state
