 ...
 *
 * $Id: SegmentedTimeline.java,v 1.9 2005/05/19 13:58:11 mungady Exp $
 *
 * Changes
 * -------
...
public class SegmentedTimeline implements Timeline, Cloneable, Serializable {
	 ...
	 * zone).
	 *
	 * If the date is not contained in the timeline (because it is not contained in
	 * the baseTimeline), a Segment that contains <code>date + segmentSize*m</code>
	 * will be returned for the smallest <code>m</code> possible.
	 *
	 * @param date
	...
	public class Segment implements Comparable, Cloneable, Serializable {
		 ...
		 *            the end of the interval.
		 * 
		 * @return <code>true</code> if this segment is contained in the interval.
		 */
		 ...
		 * search on the exceptionSegments sorted list.
		 *
		 * If the segment is not listed as an exception in our list and we have a
		 * baseTimeline, a check is performed to see if the segment is inside an
		 * excluded segment from our base. If so, it is also considered an exception.
		 *
	...
	protected class SegmentRange extends Segment {
		 ...
		 */
		public boolean inIncludeSegments() {
			for (Segment segment = getSegment(this.segmentStart); segment.getSegmentStart() < this.segmentEnd; segment
					.inc()) {
				if (!segment.inIncludeSegments()) {
					return (false);
				}
