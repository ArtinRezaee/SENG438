 ...
 *
 * $Id: PiePlot.java,v 1.25 2006/01/09 15:51:42 mungady Exp $
 *
 ...
 * @see PieDataset
 */
public class PiePlot extends Plot implements Block, Cloneable, Serializable {

	/** For serialization. */
	private static final long serialVersionUID = -795612466005590431L;

	/** The default padding. */
	public static final RectangleInsets DEFAULT_PADDING = new RectangleInsets(UnitType.RELATIVE, 0.10, 0.10, 0.10,
			0.10);
	...
	/**
	 * A flag that controls whether or not an outline is drawn for each section in
	 * the plot.
	 */
	...
	/** The number of times drawItem will be called for each item. */
	...
	public PiePlot(PieDataset dataset) {
		...
		this.baseSectionOutlineStroke = new BasicStroke(1.0f);
	...
	/**
	 * @return Returns the number of passes required to draw an item.
	 */
	...
	/**
	 * Sets the number of passed required to draw an item of the pie chart.
	 * 
	 * @param n
	 *            the number of passes required.
	 */
	public void setPassesRequired(int n) {
		this.passesRequired = n;
	}
	 ...
	 * listeners. The initial default value is 90 degrees, which corresponds to 12
	 * o'clock. A value of zero corresponds to 3 o'clock... this is the encoding
	 * used by Java's Arc2D class.
	 *
	 * @param angle
	...
	/**
	 * Returns the flag that controls whether or not the outline is drawn for each
	 * pie section.
	 * 
	 * @return The flag that controls whether or not the outline is drawn for each
	 *         pie section.
	 */
	...
	/**
	 * Sets the flag that controls whether or not the outline is drawn for each pie
	 * section, and sends a {@link PlotChangeEvent} to all registered listeners.
	 * 
	 * @param visible
	 *            the flag.
	 */
	...
	/**
	 * Sets the flag that controls whether or not label linking lines are visible
	 * and sends a {@link PlotChangeEvent} to all registered listeners. Please take
	 * care when hiding the linking lines - depending on the data values, the labels
	 * can be displayed some distance away from the corresponding pie section.
	 * 
	 * @param visible
	 *            the flag.
	 */
	...
	/**
	 * Sets the legend label tool tip generator and sends a {@link PlotChangeEvent}
	 * to all registered listeners.
	 * 
	 * @param generator
	 *            the generator (<code>null</code> permitted).
	...
	public PiePlotState initialise(Graphics2D g2, Rectangle2D plotArea, PiePlot plot, Integer index,
		...
		state.setPassesRequired(getPassesRequired());
	 ...
	 */
	public void draw(Graphics2D g2, Rectangle2D plotArea, Point2D anchor, PlotState parentState,
			PlotRenderingInfo info) {

		// adjust for the margin around the outside of the plot...
		...
		getBorder().getInsets().trim(plotArea);

		Shape savedClip = g2.getClip();
		g2.clip(plotArea);
		RectangleInsets padding = getPadding();
		padding.trim(plotArea);
	...
	protected void drawPie(Graphics2D g2, Rectangle2D plotArea, PlotRenderingInfo info) {
		...
		double gapHorizontal = plotArea.getWidth() * (0.0 + labelWidth);
		double gapVertical = plotArea.getHeight() * 0.0;
		...
		if (Math.abs(angle) > getMinimumArcAngleToDraw()) {
			...
			if (currentPass == 0) {
				if (this.shadowPaint != null) {
					Shape shadowArc = ShapeUtilities.createTranslatedShape(arc, (float) this.shadowXOffset,
	...
	public LegendItemCollection getLegendItems() {
		...
		while (iterator.hasNext()) {
			...
			if (include) {
				PieSectionLabelGenerator llg = getLegendLabelGenerator();
				String label = llg.generateSectionLabel(this.dataset, key);
				AttributedString attributedLabel = llg.generateAttributedSectionLabel(this.dataset, key);
				String description = label;
				String toolTipText = null;
				PieSectionLabelGenerator ttg = getLegendLabelToolTipGenerator();
				if (ttg != null) {
					toolTipText = ttg.generateSectionLabel(this.dataset, key);
				}
				String urlText = null;
				Paint paint = getSectionPaint(section);
				Paint outlinePaint = getSectionOutlinePaint(section);
				...
				if (attributedLabel != null) {
					item = new LegendItem(attributedLabel, description, toolTipText, urlText, true, shape, true, paint,
							true, outlinePaint, outlineStroke, false, new Line2D.Float(), new BasicStroke(),
							Color.black);
				} else {
					item = new LegendItem(label, description, toolTipText, urlText, true, shape, true, paint, true,
							outlinePaint, outlineStroke, false, new Line2D.Float(), new BasicStroke(), Color.black);
				}
