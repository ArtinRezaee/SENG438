 ...
 * Contributor(s):   -;
 *
 * $Id: PeriodAxis.java,v 1.21 2005/09/15 09:48:01 mungady Exp $
 *
 * Changes
 * -------
 ...
 * right of charts.
 */
public class PeriodAxis extends ValueAxis implements Block, Cloneable, Serializable {
	...
	/**
	 * Returns the class used to create the first and last time periods for the axis
	 * range when the auto-range flag is set to <code>true</code>.
	 * 
	...
	/**
	 * Sets the class used to create the first and last time periods for the axis
	 * range when the auto-range flag is set to <code>true</code> and sends an
	 * {@link AxisChangeEvent} to all registered listeners.
	 * 
	 * @param c
	...
	/**
	 * Sets the class that controls the spacing of the major tick marks, and sends
	 * an {@link AxisChangeEvent} to all registered listeners.
	 * 
	 * @param c
	 *            the class (a subclass of {@link RegularTimePeriod} is expected).
	 */
	...
	/**
	 * Returns the flag that controls whether or not minor tick marks are displayed
	 * for the axis.
	 * 
	 * @return A boolean.
	 */
	...
	/**
	 * Sets the flag that controls whether or not minor tick marks are displayed for
	 * the axis, and sends a {@link AxisChangeEvent} to all registered listeners.
	 * 
	 * @param visible
	 *            the flag.
	 */
	...
	/**
	 * Sets the class that controls the spacing of the minor tick marks, and sends
	 * an {@link AxisChangeEvent} to all registered listeners.
	 * 
	 * @param c
	 *            the class (a subclass of {@link RegularTimePeriod} is expected).
	...
	/**
	 * Sets the stroke used to display minor tick marks, if they are visible, and
	...
	/**
	 * Returns the size of the axis given a fixed width but no height constraint. We
	 * override the base class method to handle the multiple bands that can be
	 * displayed by this axis.
	 * 
	 * @param g2
	 *            the graphics device.
	 * @param fixedWidth
	...
	protected ArrangeResult arrangeFN(Graphics2D g2, double fixedWidth, ArrangeParams params) {
		// TODO: the following code assumes the axis is at the top or bottom
		// of the plot. It will need updating when the axis supports left
		// and right positions.
		...
		if (h < 0.0) {
			...
			h = h + Math.max(this.minorTickMarkOutsideLength, getTickMarkOutsideLength());
			for (int i = 0; i < this.labelInfo.length; i++) {
				PeriodAxisLabelInfo info = this.labelInfo[i];
				FontMetrics fm = g2.getFontMetrics(info.getLabelFont());
				h += info.getPadding().extendHeight(fm.getHeight());
			}
		}
	...
	protected ArrangeResult arrangeFR(Graphics2D g2, double fixedWidth, Range heightRange, ArrangeParams params) {
		...
		if (heightRange.contains(r.getHeight())) {
			...
			return arrangeFF(g2, fixedWidth, heightRange.constrain(r.getHeight()), params);
		}
	}
	 ...
	 * @param location
	 *            the axis location relative to the plot (<code>null</code> not
	 *            permitted).
	 * @param plotState
	 ...
	 */
	public AxisState draw(Graphics2D g2, Rectangle2D area, RectangleEdge location, PlotRenderingInfo plotState) {

		double cursor;
		if (location == RectangleEdge.TOP) {
			cursor = area.getMaxY();
		} else {
			cursor = area.getY();
		}
		...
		if (isAxisLineVisible()) {
			drawAxisLine(g2, area, location);
		}
		drawTickMarks(g2, axisState, area, location);
		for (int band = 0; band < this.labelInfo.length; band++) {
			axisState = drawTickLabels(band, g2, axisState, area, location);
		}

		drawLabel(getLabel(), g2, area, location);
		return axisState;
	 ...
	 */
	protected void drawTickMarks(Graphics2D g2, AxisState state, Rectangle2D dataArea, RectangleEdge location) {
		if (RectangleEdge.isTopOrBottom(location)) {
			drawTickMarksHorizontal(g2, state, dataArea, location);
		} else if (RectangleEdge.isLeftOrRight(location)) {
			drawTickMarksVertical(g2, state, dataArea, location);
		}
	}
	 ...
	 *            the edge.
	 */
	protected void drawTickMarksHorizontal(Graphics2D g2, AxisState state, Rectangle2D area, RectangleEdge edge) {
		List ticks = new ArrayList();
		double x0 = area.getX();
		double y0 = area.getY();
		double insideLength = getTickMarkInsideLength();
		double outsideLength = getTickMarkOutsideLength();
		RegularTimePeriod t = RegularTimePeriod.createInstance(this.majorTickTimePeriodClass, this.first.getStart(),
				getTimeZone());
		...
		while (t0 <= lastOnAxis + 1) {
			...
			if (this.minorTickMarksVisible) {
				RegularTimePeriod tminor = RegularTimePeriod.createInstance(this.minorTickTimePeriodClass, new Date(t0),
						getTimeZone());
				long tt0 = tminor.getFirstMillisecond(getTimeZone());
				while (tt0 < t.getLastMillisecond(getTimeZone()) && tt0 < lastOnAxis) {
					double xx0 = valueToJava2D(tt0, area, edge);
					if (edge == RectangleEdge.TOP) {
	...
	protected AxisState drawTickLabels(int band, Graphics2D g2, AxisState state, Rectangle2D area,
		...
		long ww = Math.round(java2DToValue(area.getX() + w + 5.0, area, location)) - axisMin;
		...
		while (p.getFirstMillisecond(this.timeZone) <= axisMax) {
			float x = (float) valueToJava2D(p.getMiddleMillisecond(this.timeZone), area, location);
			...
			if (last > axisMax) {
				...
				if ((x + bb.getWidth() / 2) > area.getMaxX()) {
					float xstart = (float) valueToJava2D(Math.max(first, axisMin), area, location);
					if (bb.getWidth() < (area.getMaxX() - xstart)) {
						x = ((float) area.getMaxX() + xstart) / 2.0f;
					} else {
						label = null;
					}
				}
			...
			if (first < axisMin) {
				...
				// so check that the label will fit before displaying it...
				Rectangle2D bb = TextUtilities.getTextBounds(label, g2, g2.getFontMetrics());
				if ((x - bb.getWidth() / 2) < area.getX()) {
					float xlast = (float) valueToJava2D(Math.min(last, axisMax), area, location);
					if (bb.getWidth() < (xlast - area.getX())) {
						x = (xlast + (float) area.getX()) / 2.0f;
					} else {
						label = null;
					}
				}
			...
			if (lastXX > 0L) {
				if (this.labelInfo[band].getDrawDividers()) {
					...
					float mid2d = (float) valueToJava2D(mid, area, location);
		...
		state.moveCursor(used, location);
		return state;
	}
	...
	/**
	 * This method does nothing in the <code>PeriodAxis</code> class.
	 *
	 * @param g2
	 *            the graphics device.
	 * @param dataArea
